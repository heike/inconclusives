...
As a result, studies which attempt to gain insight into the identification process must also navigate these complexities, ideally without becoming too complicated to \ac{themselves}.
...
\ac{The two conditional probabilities that are referred to in the diagnostic test example are $P(\mbox{Test result} | \mbox{Disease status})$ and $P(\mbox{Disease status} | \mbox{Test result})$. Bayes' rule tells us how to move from the former to the latter, assuming that we have additional information. In the case of the patient taking the test, the patient would also need to know about the baseline prevalence of the disease in her particular environment. The parallel we draw here is the following:  the examiner's decision is equivalent to a diagnostic test, and the origin of the evidence is equivalent to the disease status. In contrast to the medical example, however, we do not have information about the sensitivity and specificity of the field of firearms examination as a whole, let alone about a specific examiner.}
%work we do not know ground truth, so we cannot definitively know wheter an examiner's conclusion is correct.} 
For that reason, we need designed studies to estimate the error rates of examinations in situations where ground truth is known. \ac{Furthermore, to estimate the probability of same or different source given a specific firearm examiner's conclusion, we also need an estimate of the individual examiner's probability of correct conclusion, which can perhaps be obtained via proficiency tests.}
...
\item Random allocation of different test kits to participants, to minimize the possibility of their exchanging information.
...
While published studies vary in their treatment of inconclusive decisions when calculating and reporting error rates or accuracies,  what is reported as error rates in most studies is what we have termed source-specific conditional probabilities. 
...
in many studies, there are more same-source comparisons than what would be expected in case work. Consequently, errors that consist in false elimination  can be estimated more precisely than false identification error rates. 
