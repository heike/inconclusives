...
\subsection{Evaluating Classification Decisions}
...
Thus, our source-specific accuracy rates would be the probability that (if the evidence came from the same source) the examiner assigns the label Identification, Inconclusive, or Elimination, respectively. Figure \ref{fig:same-source}}
