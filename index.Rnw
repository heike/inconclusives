...
The most common way to assess the accuracy of a classification method is \hh{to calculate its \emph{sensitivity} and \emph{specificity}. 
...
\hh{Mathematically, we calculate the true negative rate as the conditional probability of the examiner making an elimination \emph{given} the pair under examination is from different sources. 
A conditional probability} of an event \emph{given} another event is the probability that the two events both occur, divided by the probability of the second event, \hh{specifically for the true negative rate}:
...
In all  of these assessments, the  rates are calculated by conditioning on the origin of the evidence; this process is shown graphically in \autoref{fig:FPR-specific-tree} using a decision tree to describe the comparisons which can be made conditioned on ground truth. %\hh{XXX The figure includes inconclusives - which we haven't talked about at this point. } 
...
An alternate way to evaluate a classification method is to assess 
...
Its complement, the \emph{false omission rate} (FOR) is the probability that, if an examiner makes an elimination, the evidence is actually from the same source\footnote{
In \citet{songEstimatingErrorRates2018}, the FDR is referred to as the false identification error rate, and the FOR is called the false exclusion error rate.}.
...
Note that the definition of blind studies as used above is different from the definitions of a blind study used in \citet{bunch2003comprehensive} or \citet{Duez:2017kha}; in those papers, the use of the terms blind and/or double blind do not conform to the generally accepted definitions.
...
Once a study is designed and test samples have been assessed by forensic examiners, error rates can be calculated. \svp{In the next section, we examine what error rates mean and how they are computed in the context of a generic case study. With the notation defined in the generic case study section, we then explore a number of different studies with different designs, assessing which error rates can be calculated and/or estimated, and which cannot due to deficiencies in experimental design.}
...
in the form of  \autoref{tab:generic-results}. \hh{Note that the table is nothing but a summary of all the possible combinations of examiners' decisions and actual state of the evidence as sketched out in \autoref{fig:shape-overview}. }
...
\subsection{An Overview of Study Error Rates}

\svp{ 
Tables describing the results of each of the studies surveyed in this paper, the source-specific and conclusion-specific error rates, and the overall error rates can be found in \autoref{sec:supplement}. 
...
\svp{
Our findings mirror the list of experimental design flaws identified in the 2017 addendum to the PCAST report\cite{pcast-addendum2017}. 
}
\begin{displayquote}
...
Shortly after the report was issued, two more studies were published which also meet the criteria for reliable studies in the report: \cite{keisler} and \cite{Duez:2017kha}. 
...
In what is almost certainly a coincidence, all three studies are also limited to cartridge case comparisons.
}

\svp{
There are many other studies, such as \cite{lyons}, \cite{Hamby:2018hu} (and the similarly designed \cite{fadul}), and \cite{bunch2003comprehensive}, which use sub-optimal designs. 
}
\svp{
...
In \autoref{sec:supplement}, we use statistical simulation to explore the likely number of different-source comparisons performed in \cite{bunch2003comprehensive}; this assessment includes both the effect of deductive reasoning and the sampling procedure used to create the test kits. 
...
% \svp{XXX Good, bad, and ugly study designs, and limits on possible conclusions. Mention of the prosecution bias of many common study designs which only allow for the calculation of the false positive rate.}

\svp{XXX Overall Error Rate Plot and discussion XXX}

\svp{XXX Percent of inconclusives plot and discussion}

\svp{XXX Introduce PPV/NPV as a fix for error rates and inconclusive percentages - easier to understand and use in court}
...
studies <- studies %>% mutate(
    Study = factor(Study, 
                    levels=c("Baldwin (2014)", "Keisler (2018)", "Duez (2018)", "Hamby (2019)", "Lyons (2009)"))
  )
...
<<errors, echo=FALSE, fig.width=8, fig.height=4.5,fig.cap="Percentages of missed eliminations and missed identifications by study. 95\\% Pearson-Clopper confidence intervals are drawn around the error estimates.", out.width=".8\\textwidth">>=
errors <- studies %>% 
  mutate(
...
errors %>% gather(Type, Number, AFTE, Process) %>% 
  ...
  scale_y_continuous(breaks=1:5, labels=levels(errors$Study), minor_breaks = NULL) +
...
\hh{XXX except for Fadul we are discussing all of the above studies. Fadul should be included still. Probably in the discussion of Brundage \& Hamby.}
...
\subsection{Lyons Study\cite{lyons}}

Discussion of the Lyons study:
\begin{itemize}
...
\multicolumn{1}{Y}{Concl. Total} & 1097 & 748 & \multicolumn{1}{r}{1432} & \multicolumn{1}{Y}{3270}\\\hline\hline
...
\multicolumn{1}{Y}{Conclusion Probability} & 0.3355 & 0.0101 & \multicolumn{1}{r}{0.4379} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Probability\\\hline
Same source & 0.9799 & 0.1470 & 0.0028 & 0.3333\\
Different source & 0.0201 & 0.9853 & 0.9923 & 0.6667\\\hline
\multicolumn{1}{Y}{Conclusion Probability} & 0.3355 & 0.0101 & \multicolumn{1}{r}{0.4379} & \multicolumn{1}{Y}{}\\\hline\hline
\end{tabularx}
...
\multicolumn{1}{Y}{Concl. Total} & 1508 & 207 & \multicolumn{1}{r}{805} & \multicolumn{1}{Y}{2520}\\\hline\hline
...
\multicolumn{1}{Y}{Concl. Total} & 10447 & ? & \multicolumn{1}{r}{?} & \multicolumn{1}{Y}{57502.5}\\\hline\hline
...
I & 5 & 5 & 0.0000 & 252 = ${10 \choose 5}{5 \choose 5}$\\
II & 6 & 3 & 1.0000 & 840\\
III & 7 & 1.0000 & 2 & 360\\ \hline
\end{tabular}
\end{table}
...
\multicolumn{1}{Y}{Concl. Total} & 70 & 172 & \multicolumn{1}{r}{ 118} & \multicolumn{1}{Y}{}\\\hline\hline
...
\multicolumn{1}{Y}{Concl. Total} & 177 & ? & \multicolumn{1}{r}{?} & \multicolumn{1}{Y}{?}\\\hline\hline
