...
\begin{table}
\centering
\caption{An example results table for a generic experiment where comparisons can be broken down as either from the same source or from different sources, and examiners classify each comparison as an identification, an inconclusive, or an exclusion, as in the AFTE rules of identification. Define $S = a + b + c$ as the total number of same source comparisons, and $D = d + e + f$ as the total number of different-source comparisons. 
...
Reality & Identification & Inconclusive & Exclusion & Total comparisons \\\hline
Same source & $a$ & $b$ & $c$ & $ a + b + c$ \\
Different source & $d$ & $e$ & $f$ & $d + e + f$ \\\hline
Total evaluations & $ a + d$ & $b + e$ & $c + f$ &\\\hline
\end{tabu}}
\end{table}

\subsection{Some summary name... so that we can label this bit and reference it}\label{sec:probabilities}
...
\svp{Might need to say something, somewhere, about how typical machine learning and decision theory doesn't deal in inconclusives... but I don't want to get into it in the description section below.}
\newcommand{\vbar}{$ | $~}
\begin{description}
...
\svp{Add in a table with the rates and how they are calculated using quantities in \autoref{tab:generic-results}}
...
\begin{table}
\caption{Probabilities of interest, calculated using the quantities introduced in \autoref{tab:generic-results}. In each cell, probabilities shown are of $P(X | Y)$ where $X$ is the known type of comparison (e.g. reality), either same-source or different-source, and $Y$ is the examiner's decision (one of Identification, Inconclusive, or Exclusion. }\label{tab:generic-probabilities}
{\tabulinesep=1.2mm
\begin{tabu}{|c|ccc||c|}\hline
$P(X | Y)$ & \multicolumn{3}{c||}{$Y = $Examiner Conclusion} &\\\hline
$X = $Reality & Identification & Inconclusive & Exclusion & Total comparisons \\\hline
\multirow{2}{*}{Same source} & $P(SS|Identification)$ & $P(SS|Inconclusive)$ & $P(SS | Exclusion)$ & $P(SS)$ \\
& $=\displaystyle\frac{a}{a + d}$ & $=\displaystyle\frac{b}{b + e}$ & $=\displaystyle\frac{c}{c + f}$ & $=\displaystyle\frac{S}{S + D}$\\\hline
\multirow{2}{*}{Different source} & $P(DS|Identification)$ & $P(DS|Inconclusive)$ & $P(DS | Exclusion)$ & $P(DS)$\\ 
 & $=\displaystyle\frac{d}{a + d}$ & $=\displaystyle\frac{e}{b + e}$ & $=\displaystyle\frac{f}{c + f}$ & $=\displaystyle\frac{D}{S + D}$ \\\hline
\end{tabu}
}
\end{table}
...
% 
% \begin{description}
% \item [true positive rate] (also known as {\bf sensitivity}), is the probability that two samples from the same source are identified in the examination process as a match, $P(\text{identification mde} \mid \text{same source samples})$. 
% \item [true negative rate], (also known as {\bf specificity}), is the probability that two samples from different sources result in an elimination in the examination process, $P(\text{elimination mde} \mid \text{different sources samples})$. 
% \end{description}
...
P (\text{same source} \mid \text{inconclusive}) &=&  P (\text{same source})\\
P (\text{different source} \mid \text{inconclusive}) &=& P (\text{different source}) \\[10pt]
...
In actual case work we are not able to assess the probabilities for same source or different source.  However, those probabilities are easily accessible in studies or in blinded proficiency testing. 
