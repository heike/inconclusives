...
\hh{XXX citation from evaluating PPV \citet{Casscells:1978ba, Manrai:2014fp} - along the lines: assessing PPVs is hard and unintuitive. }
...
<<hist-bunch-murphy, fig.width = 8, fig.height = 6>>=
load(here::here("data/bunch-murphy-results.Rdata"))
...
<<scatter-bunch-murphy, fig.width = 6, fig.height = 6>>=
res %>%
  # select(-iteration) %>%
