...
Mathematically, we calculate the rate of false positives as $$ FPR = P(Identification | DS) = \frac{P(Identification \text{ and } DS)}{P(DS)}.$$
...
\hh{XXX I think we have too many rates in there with too close naming. Personally, I would not distinguish between False discovery and False positive rates. But they are defined differently. Help XXX}
