...
\newcommand{\ac}[1]{{\textcolor{pink}{#1}}}
...
While these two statements seem to be contradictory, in this paper, we provide a foundation under which the two approaches can be partially reconciled in a way that provides additional insight into the examination process.
...
In what is almost certainly a coincidence, all three studies are also limited to cartridge case comparisons. We could not identify any studies which assess the error rates of bullet or toolmark examination in a manner that would produce reliable and generalizable measurements.
...
There is consistently a much larger discrepancy between the two error rates for eliminations than for identifications; this phenomena is examined in more detail in \Cref{sec:inconclusives}.
