...
=======
Another commonly used way of assessing accuracy of a classification method is to calculate the false positive rate and the false negative rate. The \emph{false positive rate} is the probability that an examiner makes an identification from evidence which is from different sources(DS), i.e. $FPR = P(Identification | DS)$.

The \emph{false negative rate} is the probability that an examiner makes an elimination from evidence which originates from the same source (SS), i.e. $FNR = P(\text{Elimination} | SS)$. In all  of these assessments, the  rates are calculated by conditioning on the origin of the evidence. \hh{XXX The figure includes inconclusives - which we haven't talked about at this point. } \autoref{fig:FPR-specific-tree} shows a decision tree describing the possible comparisons which can be made conditioned on ground truth. %\hh{XXX sorry for taking out the next paragraph - I'm not sure it helps in clarifying the math behind the tree. }
...
>>>>>>> a0a2a23045ddc723e34172dc26ffbf06b2f7a4e2
...
=======
The \emph{positive predictive value} (PPV) is the probability that, if an examiner makes an identification, the evidence is from the same source, that is, $PPV = P(SS | \text{Identification})$. 
...
The \emph{negative predictive value} (NPV) is the probability that, if an examiner makes an elimination, the evidence is from different sources, or $P(DS | \text{ Elimination})$. \svp{Its complement, the \emph{false omission rate} (FOR) is the probability that, if an examiner makes an elimination, the evidence is actually from the same source.}
...
\hh{To reduce confusion with the naming or rates and predictive values we have labelled nodes in the trees of \autoref{fig:FPR-specific-tree} and \autoref{fig:PPV-specific-tree} correspondingly. }
...
Predictive value assessments are used in assessing the implications of a particular classification label; they inform us about the likely state of the physical evidence. 
\svp{This makes the PPV, NPV, FDR, and FOR more appropriate for use when evaluating casework, where the true state of the evidence is unknown: these rates are not conditioned on unknown information, but instead use the available information (the examiner's decision) to provide updated information about the probability of an error or a correct decision.}
\svp{For the same reason, the source-specific assessments are more useful when evaluating the performance of individuals on a designed test, because ground truth is known (and thus, can be conditioned upon) in designed studies.}
...
However, if you as a patient are sent for a test which yields a positive result, what you really care about  
...
\hh{PPV and NPV, are calculated by conditioning on an examiner's conclusion and give the probability of }  
%the state of reality.
\hh{evidence being same-source or different source.}
...
\section{Assessment of error rates in case studies}
...
Same Source & $\mathbb{a}$ & $\mathbb{b}$ & $\mathbb{c}$ & $S = a + b + c$\\
Diff Source & $\mathbb{d}$ & $\mathbb{e}$ & $\mathbb{f}$ & $D = e + f + g$\\\hline
\multicolumn{1}{Y}{Conclusion Total} & $a + d$ & $b + e$ & \multicolumn{1}{r}{$c + f$} & \multicolumn{1}{Y}{$N = S + D$}\\\hline\hline
=======
\multicolumn{1}{c}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & Source Total\\\hline
Same Source & $\bm{a}$ \includegraphics[width=.25cm]{figures/a.pdf} & 
              $\bm{b}$ \includegraphics[width=.3cm]{figures/b.pdf} & 
              $\bm{c}$ \includegraphics[width=.35cm]{figures/c.pdf} & $S = a + b + c$\\
Diff Source & $\bm{d}$ \includegraphics[width=.25cm]{figures/d.pdf} & 
              $\bm{e}$ \includegraphics[width=.3cm]{figures/e.pdf} & 
              $\bm{f}$ \includegraphics[width=.35cm]{figures/f.pdf} &  $D = e + f + g$\\\hline
...
<<<<<<< HEAD
\multicolumn{1}{Y}{Conclusion probability} & $(a + d)/N$ & $(b + e)/N$ & \multicolumn{1}{r}{$(c + f)/N$} & \multicolumn{1}{Y}{1.0}\\\hline\hline
=======
...
When the goal is to assess the error rate of the examiner under the prevailing guidelines of the AFTE Theory of Identification, this is a reasonable option for assessing error rates of firearm and toolmark examiners, with the slightly odd side-effect that an examiner who marks every comparison as inconclusive does not provide any useful actionable information, but effectively has a zero error rate. \svp{XXX this is repeating something said above, though I'm not sure that's a bad thing XXX}
