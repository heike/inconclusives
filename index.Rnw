...
% \hh{XXX The next paragraph seems contradictory to the previous case. Where does this definition of double-blind come from? It seems a bit odd. }
...
Before approaching the actual studies, we will define \svp{a framework and some} basic notation to facilitate comparison of studies with different experimental designs and  explore the logic behind the numerical calculations.%order
...
Starting with the results from a generic experiment, where forensic toolmark examiners were asked to complete a total of $N$ comparisons, consisting of $S$ same-source comparisons and $D$ different source comparisons,  aggregated results of  examiners' evaluations can be reported as shown in  \autoref{tab:generic-results}. Note that the layout of the table  is nothing but a summary of all the possible combinations of examiners' decisions and actual state of the evidence as sketched out in \autoref{fig:shape-overview}. The pieces of \autoref{fig:shape-overview} are shown along with letters $a$ to $f$ in \autoref{tab:generic-results}. These letters will be used throughout the paper to calculate various accuracy rates and probabilities. 
...
% \hh{this section has a lot of repetitions. XXX are you still writing here?}\svp{I think the primary issue is that while we mentioned the mismatch above in the context of how inconclusives have been treated, we haven't really discussed why that's a problem in machine learning/algorithm classification. It's hinted at above, but... I think it needs to be *very* explicit. I'm fine combining the two somehow, but I'm not sure how to do that well - 2.1 is important to have before we get into the details, but the ``preview" at the beginning doesn't really need 2.1, and sets up the introduction $\rightarrow$ meat of paper transition really nicely. Thoughts?}
...
% \citet{Dror:2018fp} suggests treating inconclusive results as equivalent to a ``decision with certainty that the quantity and quality of information are not sufficient to draw any conclusion regarding the source". 
...
% \hh{I'm OK with that, but the 'this approach has drawn some criticism' is still hanging at the more or less stage. I'm ok with not going into the criticism until later but then this sentence has to go there as well. XXXX}
% 
...
In firearms and toolmark examination, however, this is \svp{complicated by a mismatch between the classification scheme and the state of reality.} There are two possibilities to describe the physical state of the evidence, in that the evidence is either from the same source or from different sources, but there are three \svp{primary} outcome states: the examiner can make an identification, an elimination, or an inconclusive determination\footnote{The fourth category, unsuitable for examination, is one which should be used when evaluating single pieces of recovered evidence; it does not result from the comparison of an unknown source to a known source.}.
...
\hh{Statistically, this would suggest that the decision about the amount of evidence available (inconclusive or not) would be independent of the source of the evidence (same or different source). That is, inconclusives should be equally likely in same-source and different-source assessments.}
% % \hh{XXX should we keep the criticism to the discussion?}\svp{Yes, because it makes it clear we're aware of the discussion. Otherwise we'll just get a review telling us we're too new at this because we didn't cite xyz.}
\svp{The view expressed in \citet{Dror:2018fp}} has drawn some criticism  \citep{Biedermann:2018hr,biedermannAreInconclusiveDecisions2019}\svp{, primarily on the grounds that inconclusive decisions may be more ``optimal" under formal decision-theoretic analysis than a forced binary decision. While inconclusives may be preferable in formal decision theory analyses, they pose more of a problem when assessing the accuracy and error rates of the examination process.}
...
\hh{Mathematically} there are three ways that inconclusives can be treated in calculating error rates:
...
\begin{tabularx}{.985\textwidth}{llcl}
