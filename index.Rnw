...
All \svp{five} studies \svp{with reliable designs} are also limited to cartridge case comparisons \svp{(the fifth study is FAID09 \citep{pauw2013faid}, which was published before PCAST but is a European-focused study)}. 
We could not identify any studies that assess the error rates of bullet or toolmark examination in a manner that would produce reliable \svp{error rate estimates}. \footnote{\svp{We have included FAID09 \citep{pauw2013faid}, which examines bullets and cartridge cases, but uses European examiners (who are trained to different standards and some of whom work in non-adversarial systems), and a 5-point categorical range of conclusions. 
