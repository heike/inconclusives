...
In order to assess how well the identification process is functioning, studies with known ground truth have to be employed, as casework does not allow for an assessment of correctness. 
...
\svp{In part, this variability is due to the mismatch between the evidence evaluation standards used by different forensic disciplines and the actual origin of the evidence. 
For instance, the AFTE theory of identification has 4 possible conclusions; 3 of which are typically used in comparisons\footnote{The fourth category, unsuitable for examination, is one which should be used when evaluating single pieces of recovered evidence; it does not result from the comparison of an unknown source to a known source.} 
...
For instance, \citet{Dror:2018fp} \svp{suggests treating} inconclusive results as an examiner's conscious decision to not decide on an identification or elimination. 
...
Before approaching the actual studies, we will define some basic notation to facilitate comparison of studies with different experimental designs and briefly \svp{explore} the logic behind the \svp{numerical} calculations.%order
...
Then, they assembled 20 kits with pairs of 2 cartridges from each of the 10 extractors (the knowns) and 12 questioned cartridges \hh{XXX let's distinguish between design and implementation - design involved 12 questioned cartridge cases}, such that each known corresponded to an unknown, with some replication in most of the kits. Thus, the setup of this study is similar to the Brundage-Hamby study. This study suffers from the same problems as the Brundage-Hamby study: it is a closed set study with multiple knowns and asks for identifications only. We can therefore only estimate a fraction of the relevant error rates. 
