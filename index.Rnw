...
Examiners visually classify similarity of toolmark and firearm evidence according to the AFTE theory of identification  \citep{identification} as one of identification, inconclusive or elimination. \svp{The AFTE definitions of each conclusion are provided in } \autoref{tab:afte}. Exact guidelines for this classification vary from lab to lab; some labs will exclude only on the basis of non-matching class characteristics, such as direction of the twist in rifling, land length or number of lands, or type of rifling\cite{bunch2003comprehensive}. In other labs, CMS (consecutively matching striae) as defined by  \citeauthor{biasotti}  \citep{biasotti} is used as a measure to quantify the similarity of two lands. In virtually all labs, individual characteristics used to identify \svp{similar} bullets are derived from visual assessment. %; some class characteristics may be directly measured, but these are not sufficient for individualization. 
...
In order to assess the accuracy of the identification process, we have to make use of studies where ground truth is known. 
\subsection{Studies}

\svp{Ground truth is the}
% By ground truth we mean the 
knowledge of whether two samples come from the same source or two different sources. 
%Ground truth in studies is only available to the individual setting up the test but not to any of the participants. 
\svp{In scientific studies of forensic examination, ground truth is only available to the experimenter; the participants do not have access to that information.}
\svp{Ground truth is necessary in experiments because i}n casework ground truth is not known, \svp{and thus} 
% it therefore does not allow for an assessment of  error rates 
\svp{cannot be used to assess the accuracy} of the identification process.

\svp{We refer to studies which make use of participants who were not involved in the experimental design as ``single blind" - that is, the participant knows they are participating in a study, but does not know the answers to the study.
Studies which have the additional constraint that the participant does not know the information under evaluation is part of a study at all (e.g. it is indistinguishable from normal casework) are called ``double blind". 
There is some variability in what the term ``double blind" refers to in the context of forensic studies: the use in \cite{bunch2003comprehensive, stromanEmpiricallyDeterminedFrequency2014, Duez:2017kha} does not match the use in \cite{pcast2016,spiegelmanAnalysisExperimentsForensic2013,koehlerProficiencyTestsEstimate2013}. 
\citet{kerkhoffPartdeclaredBlindTesting2018} provides a more thorough discussion of the different ways ``blind testing" has been used in forensic error rate studies.
\footnote{At least some of the confusion stems from the use of the term in medical contexts, where ``double blind" means that neither the patient nor the doctor knows whether the treatment received is the control or the treatment under investigation. 
While the terms refer to different blinding methodologies, the underlying goal is the same - to remove any biases which may cause the doctor to evaluate the patient response differently or the examiner to perform differently on a test than on regular casework.}
In this paper, we will use the term ``double blind" to refer to situations where examiners do not know that the evidence being evaluated is part of a designed experiment.
}


% \hh{XXX Blind testing - prevent differences in the process due to knowledge that one is being tested.}

% \hh{XXX Different approaches to what is considered blind}


\subsection{Study Design Considerations}

Studies must control for or systematically vary the type of implement making the pattern, manufacturing order (in many cases), maintenance procedures, ammunition or marking surface, and many other factors (see  \citet{spiegelmanAnalysisExperimentsForensic2013} for a thorough review of all of the physical factors which might be controlled for or varied in a comprehensive study). 
...
\svp{Blind testing is often recommended  \cite{pcast2016,spiegelmanAnalysisExperimentsForensic2013,koehlerProficiencyTestsEstimate2013}, because the error rates from blind testing better generalize to casework (many studies in a variety of disciplines have shown that people behave differently in situations that are known to be tests) and better assess the process of comparison as a whole (rather than the performance of an individual examiner).}
% Note that the definition of blind studies as used above is different from the definitions of a blind study used in  \citet{bunch2003comprehensive},  \citet{stromanEmpiricallyDeterminedFrequency2014}, or  \citet{Duez:2017kha}; in those papers, the use of the terms blind and/or double blind do not conform to the generally accepted definition of ``double blind" in the forensic sciences\footnote{ \citet{stromanEmpiricallyDeterminedFrequency2014} uses the NIH definition of double-blind, in which doctors administering the treatment do not know which treatment the patient is receiving, but the patient knows they are participating in a trial. This is an accurate portrayal of what double-blind means in medical studies, but in forensics, the commonly accepted definition is that a double-blind study is one in which the forensic scientist performing the evaluation does not know they are being tested. In the medical context, the doctor is also the one rating the patient's performance and may be biased by knowing which treatment the patient received. In forensics, the examiner may be biased by knowing that a comparison is part of a test. In both cases, the goal of double-blinding is to reduce external sources of bias, but the methods required to reduce the bias are different because the experimental structure and data collection methods are different.}.  \citet{kerkhoffPartdeclaredBlindTesting2018} provides a more thorough discussion of the different ways ``blind testing" has been used in forensic error rate studies.
...
\item [Study length] Most crime labs are understaffed and maintain a fairly large backlog of cases; as a consequence, examiner time is limited. While examiners should participate in these studies, they must balance the competing demands of a large and consequential workload \svp{and the benefit to FTE community}. As such, studies which require examiners to make a large number of comparisons may be less likely to find sufficient participants to generate an acceptable sample size.
\end{description}

\svp{Many of these considerations can be boiled down to limiting the examiner's knowledge about the study as much as possible, so that (ideally) the only information provided is the information in each pairwise comparison. 
...
Once a study is designed and test samples have been assessed by forensic examiners, error rates can be calculated. \svp{In the next section, we examine what error rates mean and how they are computed in the context of a generic case study. 
With the notation defined in the generic case study section, we then explore a number of different studies with different designs, assessing which error rates can be calculated and/or estimated, and which cannot due to deficiencies in experimental design.}


\subsection{End of Intro}

Unfortunately, the methods used to establish error rates experimentally and the interpretations of those error rates in the literature are highly variable. 
In part, this variability is due to the mismatch between the evidence evaluation standards used by different forensic disciplines and the actual origin of the evidence. 
For instance, the AFTE theory of identification has four possible conclusions; three of which are typically used in comparisons\footnote{The fourth category, unsuitable for examination, is one which should be used when evaluating single pieces of recovered evidence; it does not result from the comparison of an unknown source to a known source.} 
As there are more possible classifications than source categories (same or different), it is understandable that the treatment of inconclusives when calculating error rates is the subject of some debate. 
For instance,  \citet{Dror:2018fp} suggests treating inconclusive results as equivalent to a ``decision with certainty that the quantity and quality of information are not sufficient to draw any conclusion regarding the source". 
%an examiner's conscious decision to not decide on an identification or elimination. They explicitly say that inconclusives are not a decision not to decide in the section "Four Issues with Inconclusive Decisions"
Under this framework, an examiner's %default 
decision \hh{starts with} %is
inconclusive, to be refined to identification or elimination given sufficient evidence in either direction.
This approach treats inconclusives as neither errors nor correct decisions, but it does imply that in the presence of a certain amount of data, an inconclusive result is not an allowable decision and the examiner must make an identification or elimination.
\hh{XXX should we keep the criticism to the discussion?}\svp{Yes, because it makes it clear we're aware of the discussion. Otherwise we'll just get a review telling us we're too new at this because we didn't cite xyz.}
This view has drawn some criticism  \citep{Biedermann:2018hr,biedermannAreInconclusiveDecisions2019}. 
\hh{Statistically, this would suggest that the decision about the amount of evidence available (inconclusive or not) would be independent of the source of the evidence (same or different source). That is, inconclusives should be equally likely in same-source and different-source assessments.}

\hh{XXX  That's the computer sciency approach to decision theory, right?}
\hh{XXX From a statistical point of view the approach would imply independence between inconclusive decisions and source of evidence. Based on the case studies this assumption is violated.} \svp{That's a point worth making.}

\hh{XXX the approach to start with inconclusives also suggests symmetry - should we put a tease in here for later?}\svp{Yes}

%\hh{XXX should we cite a paper for each one of these decisions?}\svp{I don't know if we could find a paper that does each of these. Is there any paper you remember using the excluded from calculations option?}
...
\end{figure}
\hh{There are essentially three ways that inconclusives can be treated in calculating error rates:}
(1) inconclusives can be excluded from result calculations, (2) included as correct results  \cite{Duez:2017kha}, or (3) included as incorrect results  \cite{chumbleyValidationToolMark2010}; each of these decisions has an impact on the \hh{actual value of the error rate as well as the} interpretation of the resulting error rates.
...
\hh{We examine} %examining %previous error rate 
 technical accuracy and practical utility of  error rates \hh{published} in \hh{existing} error rate studies of firearms and toolmark examination. 
Before approaching the actual studies, we will define  basic notation to facilitate comparison of studies with different experimental designs and  explore the logic behind the numerical calculations.%order
...
\hh{\autoref{fig:shape-overview} is a sketch showing the relationship between evidence (dots) and examiners' decisions (areas). The color for both evidence and decisions corresponds to the source of evidence (different/same) and the type of decision made, respectively. The different combinations of colored dots on top of colored regions correspond to the six potential outcomes of evidence assessment and examiner decision. Obviously, not all of these outcomes are correct. } 

\svp{Starting with the} results from a generic experiment, where participants (forensic toolmark examiners) were asked to complete a total of $N$ comparisons, consisting of $S$ same-source comparisons and $D$ different source comparisons, \svp{we can report the aggregate results of } examiner evaluations using \autoref{tab:generic-results}. Note that the table \svp{layout} is nothing but a summary of all the possible combinations of examiners' decisions and actual state of the evidence as sketched out in \autoref{fig:shape-overview}. \svp{The pieces of \autoref{fig:shape-overview} are shown along with letters $a-f$ in \autoref{tab:generic-results}. These letters will be used throughout the paper to calculate various accuracy rates and probabilities. }

\begin{table}
\centering
\caption{
An example results table for a generic experiment where comparisons can be distinguished as either of same source pairs or of pairs from different sources, and examiners classify each comparison as an identification, an inconclusive, or an elimination, as  specified in the AFTE rules of identification. 
Let $S$ be the total number of same source comparisons, then $S = a + b + c$. Similarly, $D$, the total number of different source comparisons, can be written as $D = d + e + f$. The sum of $S$ and $D$ is the total number  of comparisons, $N$. 
...
\begin{tabularx}{\textwidth}{Y|YYY|Y}
...
The \emph{joint probability} of any source condition and any examiner conclusion can be found by taking the corresponding cell in \autoref{tab:generic-results} and dividing by $N$, the total number of comparisons evaluated in the study.\footnote{\svp{The joint probability of events A and B is the probabilty that both A and B occur. Here, the joint probability of an examiner identification and a same-source comparison would be equivalent to $a/N$.}} 
...
The accuracy of a classification method is  often expressed in the form of \emph{success rates} and \emph{error rates}, that is, with numerical quantities indicating how often the process produces correct or incorrect results.


\subsection{What makes an error?}\label{error-rates}
% The sets of conditional probabilities in \autoref{tab:ppv-prob} and \autoref{tab:fpr-prob} are important in the assessment of the process of evidence examination. 
\svp{In a classification problem, we would normally take the results in \autoref{tab:generic-results} and calculate various measures of accuracy and classification error.
In this situation, however, there is one complication: there are two possibilities to describe the physical state of the evidence, in that the evidence is either from the same source or from different sources, but there are three outcome states: the examiner can make an identification, an elimination, or an inconclusive determination.
Most (all?) of the typical methods of assessing classification decisions are designed for situations where there are the same number of possible decisions as there are physical classifications; 
}

However, standard evaluations of classification \svp{methods}, such as false positive and false negative rates, or sensitivity and specificity generally involve classification states which mirror reality.  \svp{XXX cite FPR,FNR, sensitivity, specificity...?}
...
\hh{The AFTE Rules of Identification introduce inconclusives as a third decision into the process. }
\svp{While identifications correspond to same-source and eliminations correspond to different-source, i}\hh{nconclusives do not correspond to a real state}.
\svp{As a result, we must decide how to account for inconclusive decisions} \hh{when assessing error rates}. 
This difference between forensic evaluation and classification problems which are more commonly evaluated provides an unfortunate opportunity for confusion when the same methods \svp{and metrics} \svp{are used to assess classification problems and } forensic decision making. 
...
In this case, the overall error rate is the sum of the false positives and the false negatives, divided by the number of overall comparisons. \svp{This approach was used in \citet{chumbleyValidationToolMark2010}, where examiners were asked to make either a positive identification or a positive elimination, and were not given the option of making an inconclusive decision. }
...
Reading this table: 
An arrangement of two rows of three boxes resembles the interior cells of \autoref{tab:generic-results}. Let \pbox{1.65em}{\small\whitesqd{}} ~correspond to a correct decision, indicating that the box is to be included in the denominator of the calculation, \pbox{1.65em}{\graysqd{}} indicate that the box is to be conditionally excluded from the calculation, and \pbox{1.65em}{\redsqd{}} correspond to an incorrect decision, indicating that the box is to be included in the numerator and denominator of the calculation. 
}

%, where \pbox{10em}{\mbox{\whitesq{a}\whitesq{b}\whitesq{c}}\\\mbox{\whitesq{d}\whitesq{e}\whitesq{f}}} are the six conditional quantities under discussion. 


\begin{tabularx}{\linewidth}{llcl}
\# & Type & Illustration & $P(Error) = $\\
0 & \pbox[l]{8em}{Inconclusives\\Not Allowed} & 
\pbox{5em}{\whitesq{a}\redsq{c}\\\redsq{d}\whitesq{f}} & 
$P(DS\text{ and }Ident.) + P(SS\text{ and }Excl.) = \displaystyle\frac{c + d}{a + c + d + f}$\\\\
%
1 & \pbox[l]{8em}{Ignore\\Inconclusives} & 
\pbox{5em}{\whitesq{a}\graysq{b}\redsq{c}\\\redsq{d}\graysq{e}\whitesq{f}} & 
$\displaystyle\frac{P(DS\text{ and }Ident.) + P(SS \text{ and } Excl.)}{P(Ident.) + P(Excl.)} = \displaystyle\frac{c + d}{a + d + c + f}$\\\\
%
2 & \pbox[l]{8em}{Inconclusives\\as Correct} & 
\pbox{5em}{\whitesq{a}\whitesq{b}\redsq{c}\\\redsq{d}\whitesq{e}\whitesq{f}} & 
$P(DS\text{ and }Ident.) + P(SS\text{ and }Excl.) = \displaystyle\frac{c+d}{N}$\\\\
%
3 & \pbox[l]{8em}{Inconclusives\\as Incorrect} & 
\pbox{5em}{\whitesq{a}\redsq{b}\redsq{c}\\\redsq{d}\redsq{e}\whitesq{f}} & 
$P(DS\text{ and }Ident.) + P(SS\text{ and }Excl.) + P(Inconcl.) = \displaystyle\frac{b + c + d + e}{N}$\\
\end{tabularx}\label{tab:overall-err-rate}
\end{fmpage}



One reasonable solution to the inconclusive problem would be to calculate the probability of an error, \svp{provided} that the classification was not inconclusive, as in option 1 of \autoref{tab:overall-err-rate}. 
...
This solution does entirely ignore inconclusives, though, and as they are part of the process and can be included in examiner testimony (usually phrased as ``could not be excluded"), this elimination from consideration is not appropriate if the goal of a study is to assess the error rate of the entire evaluation process. 
 \citet{koehlerProficiencyTestsEstimate2013} recommends using the option 1 approach for calculating error rates while  also tracking rates of inconclusive identifications ``for other purposes".
\svp{This would be equivalent to using the quantity in option 1 to assess error rates while tracking $P(Inconclusive)$ or possibly, separately tracking $P(Inconclusive \text{ and } SS)$ and $P(Inconclusive \text{ and } DS)$.}
...
When the goal is to assess the error rate of the examiner under the prevailing guidelines of the AFTE Theory of Identification, this is a reasonable option for assessing error rates of firearm and toolmark examiners, with the slightly odd side-effect that an examiner who marks every comparison as inconclusive does not provide any useful actionable information, but effectively has a zero error rate.
...
If e.g. a firearm does not mark well, a firearms examiner might not be able to make an identification or an elimination \svp{for reasons having nothing to do with the examiner's skill}. 
...
This requires that we consider the errors from the perspective of the real-world state: two pieces of evidence are  either from the same source or originate from different sources; an inability to distinguish between those options is an error in the \emph{process} even it if is not an error made on the part of the examiner. \svp{This is consistent with an observation in  \citet{koehlerFingerprintErrorRates2007} in an assessment of errors in fingerprint identification (though he goes on to use an approach more consistent with option 1). }
...
To illustrate why this is the case, consider that an examiner may, upon inspection of a fired bullet, find that there are insufficient individualizing characteristics to make an identification. \svp{XXX we need to make this a comparison example XXX}  \svp{XXX Also, can we get a better definition of unsuitable? XXX}
This does not reflective negatively on the examiner's skill; it is merely a consequence of the contact made between the \svp{ammunition and the gun components} %change b/c we have cartridges in there too
...
\svp{Some studies screen for this possibility, removing evidence with insufficient identifying marks before the study is conducted.}
\svp{In other studies, however, screening was intentionally not performed, so as to better mimic actual casework conditions. 
...
\svp{Additionally,} \hh{for 3234 comparisons, FTEs evaluated how many of the known cartridge cases were usable for an evaluation: all three specimens were used in 3018 cases, two were used in 207 cases, and only one was used in nine cases.}
\svp{The Baldwin study is fairly unique in this regard: most studies make some effort to control the quality of the evidence sent out for assessment.}
\svp{Studies which tightly control the evidence quality and presence of individualizing marks would under-estimate the process error (though the estimates of examiner error are not necessarily affected) XXX not sure this is true - need to think through it...}


\svp{There are, however, additional methods for assessing the reliability of a classification method, leveraging conditional probabilities.
...
Certain conditional probabilities have the advantage of not requiring inconclusives to be handled as errors or correct decisions. 
In addition to removing many of the complications of using the AFTE theory of identification on a binary source problem, these conditional probabilities offer some insights into the use of firearms and toolmark analysis during testimony. 
XXX this is not quite what I'm going for, but it's closer than not having it... XXX
}
...
The most common way to assess success rates of a classification method is to calculate its \emph{sensitivity} and \emph{specificity}. 
...
Mathematically, we calculate the true negative rate as the conditional probability of the examiner making an elimination \emph{given} the pair under examination is from different sources. 
...
\hh{Complementary to accuracy rates, we define two error rates of a classification method:}
...
\hh{Note that we are keeping the definition of error rates a bit vague on purpose. 
% We will pick up the discussion of how to precisely define error rates in~\autoref{error-rates}.
}
\svp{As in \autoref{sec:error-rates}, there are multiple ways to define an error; the concept holds for both option 1 and option 2 treatment of inconclusives with regard to errors.}
%an examiner makes an identification from evidence which is from different sources (DS), $FPR = P(\text{Identification} | DS)$.

%The \emph{false negative rate} is the probability that an examiner makes an elimination from evidence which originates from the same source (SS), $FNR = P(\text{Elimination} | SS)$. 
In all  of these assessments, the rates are calculated by conditioning on the \emph{origin of the evidence}; this process is shown graphically in \autoref{fig:FPR-specific-tree} using a decision tree to describe the comparisons which can be made conditioned on ground truth. Calculations for each value are shown in \autoref{fig:pv-prob}.
...
\end{tabularx}
\end{table}
A google sheets workbook which will perform all of these calculations \svp{given counts $a, b, c, d, e, \text{ and } f$} is available at \url{http://bit.ly/FTE-error-rate-worksheet}.
...
Predictive value assessments are used in assessing the implications of a particular classification label; they inform us about the \emph{likely state of the physical evidence}. 
\svp{
This makes the PPV, NPV, FDR, and FOR more appropriate for use when evaluating casework (or in a courtroom setting), because the true state of the evidence is unknown.
While mathematically it makes sense to condition on unknown quantities, in practice it is much more informative to condition on quantities whose values are known, because this allows us to obtain probabilities that are more accurate for the specified situation.
}
\svp{Conversely, source-specific assessments may be useful when evaluating the performance of individuals on a designed test, because ground truth is known (and thus, can be conditioned upon) in designed studies.}

\subsection{Practical impact of error rates}

Mathematically, the primary difference between the decision tree diagrams in Figure \ref{fig:FPR-specific-tree} and \ref{fig:PPV-specific-tree} is the conditioned variable. 
...
From a practical point of view, whether source-specific or decision-specific rates are of interest \svp{depends upon } where in the overall (legal) decision process we are: 
\begin{itemize}
\item as a firearms and toolmark examiner or a lab director, we are interested in assessing accuracy based on ground truth, i.e. we are interested in source-specific rates to answer questions of the form: given this evidence, are  trained firearms examiner more likely than trainees to come to the correct conclusion? (Case studies later show that yes, that is the case.) \svp{Alternately, lab directors might want to know if all of the examiners employed by the lab are performing above a certain threshold.}
\item as part of courtroom proceedings, we -- the jury, lawyers, and judges -- are interested in assessing accuracy of testimonies, i.e. we are interested in conclusion-specific rates to answer questions of the form: given an expert testified to the evidence as being an identification, what is the probability that the evidence actually does come from the same source? \svp{In this situation, we can update the overall error rate by conditioning on the examiner's conclusion, providing more specific evidence about the scenario under scrutiny in court. As we only know the examiner's testimony (and not ground truth), we must work with the decision-specific rates. }
\end{itemize}
...
\svp{This probability is the positive predictive value, or PPV of the test, and is equivalent to the decision-specific probability P(SS|Identification). }
...
\svp{In case work, because ground truth is not known, we cannot definitively know that an examiner's evaluation is correct. 
...
As we rely on these studies to generalize to casework, it is of great importance that the studies be well designed so as to allow for the estimation of any of the success or error rates discussed above.
}


In the next section, we will examine the results from a number of studies of the error rate of firearms examinations, identifying the results which are reported \svp{and presenting error rates and conditional probabilities derived from the results of each study for comparison and discussion.}
...
\svp{
Here, we summarize the primary findings from our survey of the available literature; while we did not attempt to include every study ever performed, we did aim to include studies which are frequently cited in research or legal proceedings, as well as most of the studies mentioned in the PCAST and NAS reports concerning firearm and toolmark analysis.}
...
\svp{In the next sections, we discuss the design and results of each study, including calculation and comparison of source- and decision-specific error rates. }

\subsection{Consequences of Study Designs for Error Rate Estimation}
\svp{Our survey of the most commonly cited studies reveals a list of experimental design concerns similar to those identified in the 2017 addendum to the PCAST report \cite{pcast-addendum2017}}. 
...
The PCAST response, isued in  \citeyear{pcast-addendum2017}, identifies only one study \cite{Baldwin:2014bb} as appropriately designed to evaluate the validity and reliability of firearms analysis methods. 
...
\svp{As a result, admissibility hearings on other types of toolmark evidence must cite studies which use less reliable designs: }  \citet{lyons},  \citet{Hamby:2018hu} (and the similarly designed  \citet{fadul}), and  \citet{bunch2003comprehensive}. 
Of these, the study by  \citeauthor{bunch2003comprehensive} is perhaps the least flawed: while it has internal dependencies due to the inclusion of multiple knowns, and as a result, the number of same-source and different-source comparisons cannot be fully determined, it does have one feature not found in even the well designed studies.
Specifically,  \citeauthor{bunch2003comprehensive} varies the composition of the test kits, so that no kit had the same composition in terms of same and different-source comparisons. This ensures that even if examiners discuss the tests, they cannot gain any additional information from such discussions. 
...
It might be possible to modify the answer sheet so that examiners record each pairwise comparison.
Using this mechanism, it might be possible to at least assess the number of different-source comparisons performed. 
\svp{However, revising the answer sheet would still not prevent the use of deductive reasoning, which gives examiners an advantage in the error rate studies that does not exist in casework.}
...
In addition to the design issues which plague the closed-set studies, the execution of some of these studies makes the data even less reliable. Methodological issues, such as those described in  \cite{lyons}, where test sheets were returned to participants who misunderstood instructions, cast even more doubt on the utility of error rates derived from such studies and their ability to generalize to casework. 

Finally, there are some methodological constraints that are common to most studies. Of the studies assessed in this paper, only  \citet{bunch2003comprehensive} uses a variable percent of same and different source comparisons; without varying this percentage, it is possible that examiners could compare notes and gain additional information about the study results. Many studies (but not all) pre-screened kit components to ensure that sufficient markings were present to allow for identification - this differs from casework and artifically inflates the rate of successful identifications and eliminations. In almost all studies, participants use lab rules for determining whether eliminations could be made on individual characteristics; while  \citet{Baldwin:2014bb} instructed participants to use a uniform set of rules, many participants reported that they did not adhere to these guidelines.
...
Process errors, however, occur at much higher rates and are particularly concerning for missed eliminations. 
...
Some of the discrepancy between the examiner and process error rates for missed eliminations can be explained by lab policies that only allow eliminations based on mismatches in class characteristics. 
It should be noted that these policies directly contradict the AFTE Theory of Identification, which allows for eliminations based on ``significant disagreement of discernible class characteristics and/or individual characteristics". 
...
<<errors, echo=FALSE, fig.width=8, fig.height=3,fig.cap="Percentages of missed eliminations and missed identifications by study. 95\\% Pearson-Clopper confidence intervals are drawn around the error estimates. Missed eliminations cannot be calculated for closed-set studies.", out.width=".8\\textwidth">>=


errors %>% gather(Type, Number, AFTE, Process) %>% 
  mutate(Type = factor(Type, levels=c("AFTE", "Process"))) %>%
  ...
  facet_grid(.~Error, scale = "free_x") + #, space="free") +
  theme_bw() +
  theme(legend.position = "bottom") +
  xlab("Error Percentages") +
  xlim(c(0, NA)) +
...
From \autoref{fig:source-spec-cis}, it is apparent that examiners are extremely good at working with same-source evidence: there are relatively few inconclusives, almost no missed identifications, and very few false eliminations. 
...
\subsection{Decision-specific error rates}
...
Same-source pairs should be assessed as inconclusive at the same frequency as same-source comparisons in the study.
It is very clear that the probability of same-source evidence given an inconclusive evaluation is much lower than the target probability.
The decision-specific probabilities provide indications that the distribution of inconclusives is much different than hypothesized in \autoref{fig:shape-overview}. %purposefully understated.
}
<<cis, echo=FALSE, fig.width=8, fig.height=4.5, fig.cap="Pearson-Clopper 95\\% confidence intervals for the probability of same source evidence given an examiner's conclusion. Expected values for each of the probabilities as discussed in \\autoref{sec:ideal} are shown as grey targets. Some of the studies do not allow an assessment of all of the predictive values.", out.width=".8\\textwidth">>=
...
\section{Inconclusives}\label{sec:inconclusives}
Prior to that, however, it may be useful to develop one more idea: in a perfect world, 
%with examiners who are all-knowing, 
what should the results of these studies look like?\svp{XXX This could be a bit better transition XXX}
\hh{XXX we don't need all knowing examiners - then we don't have inconclusives e.g.}

\subsection{Ideal study probabilities}\label{sec:ideal}
...
\svp{In the Baldwin study  \cite{Baldwin:2014bb}}, \hh{we would expect the probabilities for same and different source given an inconclusive result to be: }
\begin{eqnarray*}
...
\hh{In fact, even if inconclusives and eliminations are combined,} \svp{that is, if we used a binary classification scheme,} \hh{the error rate of making a false elimination is 0.0069, only a third of the error of a false identification:}
...
\svp{Bias against the defense in inconclusives mirrors biases in admissibility of forensic evidence  \cite{morenoWhatHappensWhen2004, epsteinPreferringWiseMan2014, roachFORENSICSCIENCEMISCARRIAGES2009}}

\svp{Focus of the discipline on the issues with identifiability, or error rates of the examiners, rather than the treatment of the entire process}
...
\noindent\begin{fmpage}{\textwidth}\centering % Checked for accuracy on 2020-04-24
\captionof{table}{Baldwin study results, conclusion-specific and source-specific probabilities, and reported overall error rates. }\label{tab:baldwin-summary}
...
& \# Same Source Comparisons & \# Different Source Comparisons & \\\hline
...
\noindent\begin{fmpage}{\textwidth}\centering % Checked for numerical accuracy 2020-04-24
\captionof{table}{{\large\bf Isolated Pairs Research Study} \label{tab:keisler-summary}}
...
\noindent\begin{fmpage}{\textwidth}\centering % Checked numbers 2020-04-24
\captionof{table}{Duez study results, conclusion-specific and source-specific probablities, and reported overall error rates. For the conditional probabilities, 
...
\noindent\begin{fmpage}{\textwidth}\centering % Checked numbers 2020-04-24
\captionof{table}{Evaluations, conditional error rates, and overall error rates from the combined Brundage-Hamby studies. Note the focus on identifications.}\label{tab:hamby-summary}
...
\noindent\begin{fmpage}{\textwidth}\centering % Checked numbers 2020-04-24
\captionof{table}{}\label{tab:lyons-summary}
...
<<histogram, fig.width=6, fig.height = 3, outwidth=".5\\textwidth", warning=FALSE, message=FALSE, fig.caption="Histogram of the total number of different source comparisons before an identification is made.", fig.label="fig:histogram">>=
library(tidyverse)
