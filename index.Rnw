...
Only cartridge cases caught with the cartridge catching device (XXX name?) were used to assemble kits. No other pre-scanning of usability for comparisons was done. 
...
Same source & 0 & 4 & 1508 & 1512 \\
Different source & 805 & 203 & 0 & 1008 \\\hline
\end{tabu}
}
\end{table}

The resulting set of conditional probabilities is shown in \autoref{tab:keisler}.
...
\autoref{tab:duez-totals} shows results from 56 evaluations (46 examiners, 10 trainees).
...
Different source & 0+2 & 12+13 & 80+5 & 92 + 12 \\\hline
\end{tabu}
}
\end{table}


The resulting conditional probabilities are shown in \autoref{tab:duez}. 
...
\end{table}
Again, the probability of same source pairs given examiners reported an inconclusive result is much lower than the expect 25\% based on the setup of the study.
