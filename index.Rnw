...
\svp{Studies must control for or systematically vary the type of implement making the pattern, manufacturing order (in many cases), and similar factors, while also controlling the amount of information available to the examiner about the study design and structure. 
When this information is not strictly controlled, examiners may use strategies (e.g. deductive reasoning) that are useful for the test but that do not apply to casework.}
...
\svp{Inconclusives can be excluded from result calculations, included as correct results, or included as incorrect results; each of these decisions has an impact on the interpretation of the resulting error rates.}
...
\svp{examining previous error rate studies to assess the technical accuracy and practical utility of the reported error rates. }
\svp{Before approaching the actual studies, it may be useful to explain the logic behind the probability calculations and define some basic notation to facilitate comparison of studies with different structure.}\svp{XXX probability hasn't been discussed yet - need to nail down terminology between probability and error rate and conditional error rate XXX}

\subsection{Evaluating Classification Decisions}\label{sec:evaluating-classification-decisions}

\svp{When an examiner assesses a set of evidence (generally, a pair of items, one of known provenance and one of unknown provenance) and decides whether the evidence is an identification, inconclusive, or an elimination, they are \emph{classifying} the evidence according to a classification scheme. 
...
\svp{Consider, for a moment, the problem of a toddler sorting colored blocks into red, green, and blue piles. 
...
\svp{We will term these \emph{conditional probabilities} {\bf source-specific} probabilities (probabilities depending on the real class of the evidence) and {\bf conclusion-specific} probabilities (probabilities which depend on the conclusion or label assigned to the evidence).}

\svp{Conditional probabilities or error rates are frequently used to describe evidence, and must be interpreted correctly. 
...
\svp{Translating our toddler block scenario to forensic evidence, we have two states of reality: the evidence originates from the same source, or the evidence originates from different sources. 
...
Figure \ref{fig:source-specific-tree} shows a decision tree for the source-specific accuracy rates.}

\begin{figure}
\centering
\includegraphics[width=\textwidth]{figures/Source_Specific_Tree_Diagram.pdf}
\caption{Tree Diagram illustrating the logic of source-specific error rates. We must first decide whether we are interested in same-source or different-source rates, and then examine the probabilities for each of the possible labels: Identification, Inconclusive, and Elimination.}\label{fig:source-specific-tree}
\end{figure}

\svp{During testimony, an examiner might report that the probability of making an identification is 98\% when comparing two items from the same source, or that the probability of making an identification if the items originated from different sources (e.g. a false identification) is 0.05\%. 
In both cases, these are source-specific conditional probabilities.}
\svp{The problem with source-specific conditional probabilities is that we do not know, when considering evidence in a particular case, whether we have same-source or different-source evidence - we do not know ground truth for casework.
...
\caption{Tree Diagram illustrating the logic of conclusion-specific error rates. We must first select which conclusion we are interested in, and then examine the probability of each state of reality (same source or different sources) given that conclusion.}\label{fig:conclusion-specific-tree}
\end{figure}

\svp{
\autoref{fig:conclusion-specific-tree} shows a tree diagram which can be used to examine the probability that reality (same or different source) corresponds to the label assigned by the examiner.
}
