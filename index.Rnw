...
\svp{
Note that we still cannot directly calculate the probabilities for any individual case or evidence: ground truth is still not knowable; however, the results of designed studies can be used to give information which is tailored to the situation in a particular case. 
