...
Examining the experimentally determined error rates for firearms and toolmark identification leads to a disturbing realization. \svp{XXX Mattijssen is directly counter to this. How do we want to address it? Blame AFTE rules? The adversarial system? We still need to smooth this discussion out a bit. XXX}
