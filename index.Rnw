...
The \emph{negative predictive value}(NPV) is the probability that, if an examiner makes an exclusion, the evidence is from different sources, or $P(DS | Exclusion)$. 
Predictive value assessments are extremely useful in understanding the implications of a particular classification label on the likelihood of reality. 
We can map out the conditional hierarchy using a decision tree, shown in \autoref{fig:PPV-specific-tree}. 
While PPV and NPV are typically calculated to assess success rather than error, we can of course consider the probability of same source evidence given an examiner exclusion (FOR, false omission rate), or of different-source evidence given an examiner identification (FDR, false discovery rate). 
