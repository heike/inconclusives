...
\svp{While it makes sense to condition on unknown quantities in a mathematical sense,} in practice it is much more informative to condition on quantities with known values, because these probabilities are immediately applicable in the specified situation.
