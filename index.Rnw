...
In what is likely a coincidence, all three studies are also limited to cartridge case comparisons.
}
...
<<errors, echo=FALSE, fig.width=8, fig.height=4.5,fig.cap="Overall error rates.", out.width=".8\\textwidth">>=
errors <- studies %>% 
  mutate(
...
errors %>% gather(Type, Number, AFTE, Process) %>% 
  mutate(Type = factor(Type, levels=c("Process", "AFTE"))) %>%
  ggplot(aes(weight = Number/Comparisons*100, x = Study)) +
  geom_bar(aes(fill=Type), colour = "grey20") +
  facet_grid(.~Error, scale = "free_x") +
  theme_bw() +
...
<<errors2, echo=FALSE, fig.width=8, fig.height=4.5,fig.cap="Overall error rates. Approach 2. Needs some work, but I like it better than the bars.", out.width=".8\\textwidth">>=
errors %>% gather(Type, Number, AFTE, Process) %>% 
