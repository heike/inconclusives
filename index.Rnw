...
\autoref{tab:overall-err-rate} shows several different possible error rate calculations, using the general structure of \autoref{tab:generic-results}.
...
One reasonable solution to the  would be to calculate the conditional probability of an error given that the classification was not inconclusive, as in option 1 of \autoref{tab:overall-err-rate}. Note that if we condition on the decision being either identification or elimination, the probability calculations for scenarios 0 and 1 in \autoref{tab:overall-err-rate} are identical; the only difference between the two is the setup of the scenario: In \autoref{eqn:overall-error-conditional} we allow for inconclusives as a decision category but restrict them from the calculation of the error rate entirely. This solution does entirely ignore inconclusives, though, and as they are part of the process and can be included in examiner testimony (usually phrased as ``could not be excluded"), this elimination from consideration is not entirely justifiable. 

Under AFTE's Theory of Identification, inconclusive classifications are not errors. Option 1 in \autoref{tab:overall-err-rate} demonstrates how some studies perform error rate calculations using this principle: inconclusives are included as identifications in false positive rate calculations, and also included as eliminations when calculating the  however, this leads to an apparent contradiction, because they are also not correct identifications; as a result, while inconclusives count in the denominator in both calculations, they are not generally included in the numerator of either the error rate or the success rate. This is not an issue if properly handled, but, typically papers use methods which were intended for binary classification, reporting the false negative rate and the false positive rate; this leads to error rates which are unintentionally misleading and unrealistically low. These error rates can be interpreted as the errors made by the examiner, rather than the errors made in the \emph{process} - if striae are not actually engraved on a bullet because of the firing process, or if RACs are not visible in shoe prints, this is not an error made by the examiner, but it is an error in that evidence which originates from the same source cannot be used to make an identification. \svp{this sentence doesn't fit but also needs to go in here somewhere.}


Another reasonable solution is to \svp{XXX need to explain scenario 3 and conclude it's the best option to cover the errors in the examination process as a whole.}
...
% \text{(misleading)} P(Error) = P(Elimination \text{ and } SS) + P(Identification \text{ and} DS) = \frac{c + d}{a + b + c + d + e + f}
% \end{equation}
...
% 
% \begin{table}\centering
% \caption{A common method for calculating the error rate of forensic identifications when examiners can decide a comparison is an identification, inconclusive, or an eliination. Rates are calculated 
...
& \# Evaluations & Elimination & \multirow{2}{1in}{Inconclusives or no response} & Identification\\
\\ \hline
...
Each of the above evaluations is  based on a comparison of one questioned cartridge case to three known same source  cartridge cases.

\hh{useful information, but not sure where to put it}
...
It seems that firearms examiners are more willing to bet on identifications than on eliminations. In fact, even if inconclusives and eliminations are combined, the error rate of making a false elimination is 0.0069, only a third of the error of a false identification:
