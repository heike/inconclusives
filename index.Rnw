...
\multicolumn{1}{Y}{Conclusion probability} & $(a + d)/N$ & $(b + e)/N$ & \multicolumn{1}{c}{$(c + f)/N$} & \multicolumn{1}{Y}{1.00}\\\hline\hline
\end{tabularx}
\end{table}
...
The resulting set of conditional probabilities is given in \autoref{tab:keisler-summary}. Again, inconclusive results are much more indicative of different source results than they should be  based on the setup of the study; we would expect same source results with a probability of 0.60 given an inconclusive result. 
