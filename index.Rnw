...
\svp{Unfortunately, this is not what we find. As shown in \autoref{fig:asymmetric-errors}, it is clear that} examiners are willing to gamble on an indentification much more than they are willing to gamble on an elimination. 
...
Whatever the source of the bias, however, one thing is clear: the discrepancy between the decision-specific conditional probabilities corresponding to errors is fundamentally unjust, in the opposite direction of the ideal expressed by Blackstone.
This is a systemic problem across many different studies of different firearm and tool marks, and one that must be addressed for forensic firearms and toolmark examination to be used equitably. XXX this is weak.
}
