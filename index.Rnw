...
\begin{abstract}
In the past decade, and in response to the recommendations set forth by the \citet{nrc:2009}, scientists have conducted several black-box studies that attempt to estimate the error rates of firearm examiners. Most of these studies have resulted in vanishingly small error rates, and at least one of them \citep{Baldwin:2014bb} was cited by the President's Council of Advisors in Science and Technology (PCAST) during the Obama administration, as an example of a well-designed experiment. What has received little attention, however, is the actual calculation of error rates and in particular, the effect of inconclusive findings on those error estimates. The treatment of inconclusives in the assessment of errors has far-reaching implications in the legal system. Here, we revisit several black-box studies in the area of firearms examination, \svp{investigating} \hh{their treatment of inconclusive results.} \svp{It is clear that there are stark differences in the rate of inconclusive results in regions with different norms for training and reporting conclusions.} \hh{More surprisingly, the rate of inconclusive decisions for materials from different sources} \svp{is notably higher than the rate of inconclusive decisions for same-source materials in some regions}. To mitigate the effects of this difference we propose a unifying approach to the calculation of error rates that  is directly applicable in forensic laboratories \svp{and in legal settings}.
\end{abstract}
...
\begin{itemize}
\item[] \hh{
there might be something intrinsically more difficult in determining eliminations - different training? Algorithms don't seem to have problems with eliminations
}
\item[] \hh{case studies  deal with largely non-damaged materials - some results based on case studies might not be transferrable to casework}
\end{itemize}
...
In other labs, CMS (consecutively matching striae) as defined by   \citet{biasotti} is used as a measure to quantify the similarity of two lands. 
...
\begin{description}
\item [Reference and Questioned Samples] Most studies are set up to mirror casework, i.e.\ there is a set of samples of known origin, \svp{used for reference}, and a set of questioned samples of unknown origin. The main objective of a study is to determine for a pair of known and questioned samples if they share the same source or come from different sources.
...
The use in \citet{bunch2003comprehensive, stromanEmpiricallyDeterminedFrequency2014, Duez:2017kha} does not match the use in \citet{pcast2016,spiegelmanAnalysisExperimentsForensic2013,koehlerProficiencyTestsEstimate2013}.
...
target_plot(100, 50, 10, 10, 50, 100, side_labels = c("Different Source", "Same Source")) 
@
...
Different source & \begin{minipage}[c]{1em}$\bm{d}$ \end{minipage}\begin{minipage}[c]{1em} \includegraphics[width=.25cm]{figures/d.pdf} \end{minipage}& 
              ...
              \begin{minipage}[c]{1em}$\bm{f}$ \end{minipage}\begin{minipage}[c]{1em} \includegraphics[width=.35cm]{figures/f.pdf} \end{minipage}&  $D = d + e + f$\\\hline
...
Note that this approach is exploitable in a strange way: to give a very extreme example, an examiner could report inconclusive decisions for every evaluation over the rest of their career and never make an error. \svp{\citet{drorErrorErrorRate2020} point out this discrepancy in many forensic disciplines and describe the effects of this policy on an individual assessment level as well as a system level.}
However, when the goal is to assess the error rate of the examiner \emph{under the prevailing guidelines of the AFTE Theory of Identification}, Option 2 \svp{may be} a reasonable option for assessing error rates of \svp{individual} firearm and toolmark examiners.
...
\svp{As discussed in \citet{drorErrorErrorRate2020}, additional statistics that provide context to error rates can be extremely beneficial in court.}
...
\svp{In the final column of the table, the total number of comparisons is shown - this number forms the denominator for each cell when calculating source-specific probabilities.}
%In the last column of the table, the marginal probabilities $P(X)$ are shown; 
These marginal \svp{total comparison numbers} are determined by the experimental design.
}\label{tab:fpr-prob}
\begin{tabular}{|r|ccc|c|}
\multicolumn{5}{c}{\bf Source-specific Probabilities}\\\cline{2-5}
\multicolumn{1}{r|}{} & Identification & Inconclusive & Elimination & Total \# Comparisons \\\hline
Same source & $a/S$ & $b/S$ & $c/S$ & $S$\\\hline
Different source & $d/D$ & $e/D$ & $f/D$ & $D$\\\hline
...
The \emph{positive predictive value} PPV is the probability that a pair of evidence items have the same source, given that the examiner has made an identification, i.e.\ $P(SS | \text{Identification})$. 
...
To reduce confusion with the naming of rates and predictive values we have labeled nodes in the trees of \Cref{fig:FPR-specific-tree} and \Cref{fig:PPV-specific-tree} corresponding to the rates defined in the text.
...
\svp{In the final row of the table, the total number of comparisons is shown - this number forms the denominator for each cell when calculating decision-specific probabilities.}
...
\multicolumn{4}{c}{\bf Decision-Specific Probabilities}\\\cline{2-4}
\multicolumn{1}{r|}{} & Identification & Inconclusive & Elimination \\\hline
Same source & $a/(a+d)$ & $b/(b+e)$ & $c/(c+f)$ \\
Different source & $d/(a+d)$ & $e/(b+e)$ & $f/(c+f)$ \\\hline
Total \# Decisions & $a + d$ & $b + e$ & $c + f$ \\\hline
\end{tabular}
\end{table}
...
Note that we still cannot directly calculate the probabilities for any individual case or evidence: ground truth is still not knowable; however, the results of designed studies are used to give information tailored \svp{to a specific decision scenario}. 
...
The two conditional probabilities that are referred to in the diagnostic test example are the probability of a positive or negative test result given the disease is present or not: $P(\mbox{Test result} | \mbox{Disease status})$ and the probability that somebody has the disease or not, given a specific test result: $P(\mbox{Disease status} | \mbox{Test result})$. 
...
The studies included in this section are not an exhaustive list of every error rate study ever performed; rather, we have included the most central and most cited studies, as well as several of the studies mentioned in the PCAST and NRC reports concerning firearm and toolmark analysis. A brief description of each study is provided here for reference purposes, with summary tables of results and conditional probabilities provided in \Cref{sec:supplement}. \svp{A spreadsheet with the same study-specific error rates and calculations is available online at \url{http://bit.ly/fte-study-error-rates} for reference purposes.}

\begin{description}
\item[Baldwin] \citep{Baldwin:2014bb} is a study of Ruger SR9 cartridge cases. This is an open-set study. Each participant is asked to  evaluate 15 test sets. Each test set consists  of 3 reference cartridge cases and 1 questioned cartridge case. In ten of the 15 test sets the questioned cartridge case matched the references, in five tests the questioned cartridge case was from a different source.

\item[Keisler] \citep{keisler} is a study of cartridge case comparisons. It is an open set study. Participants were asked to evaluate 20 test sets. Each test set consisted of one reference and one questioned cartridge cases. In twelve of the 20 sets the questioned cartridge case matched the reference, in the other eight sets the questioned cartridge case came from a differet source. A total of 126 examiners participated.

\item[Brundage-Hamby]  \citep{Hamby:2018hu} is a closed-set study of 10 consecutively manufactured Ruger P-95 barrels. Sets consisting of 10 pairs of reference bullets and 15 questioned bullets were provided to 507 study participants over the course of several decades of data collection. 

\item[Lyons] \citep{lyons} is a closed-set study of 10 consecutively manufactured Colt 1911A1 extractors. Participants received a set of 10 pairs of reference cartridge cases and 12 questioned cases. The design of this study is similar to  \citep{Hamby:2018hu}.

\item[Bunch] \citep{bunch2003comprehensive} is an open-set study of consecutively manufactured Glock breech faces. In this study, 8 participants from the FBI laboratory received 10 cartridge cases each  and were required to evaluate all possible pairwise comparisons. The number of same-source and different-source cartridge cases varied by test kit, ranging from 10 same-source cartridges to 10 different-source cartridges.

\item[Fadul] \citep{fadul} is a closed-set study of the breech face striations/impressions produced by 10 consecutively manufactured slides. Participants received a set of 10 pairs of reference cartridge cases and 15 questioned cases. The design of this study is similar to  \citep{Hamby:2018hu}.

\item[Duez] \citep{Duez:2017kha} is an open-set study of breech face comparisons using virtual microscopy. Each of 56 participants (46 trained, 10 trainees, primarily from the US and Canada) were asked to evaluate the same two test sets consisting of three reference scans and four questioned scans. In one set, all questioned scans were from the same source as the reference scans; in the second set, there were two different-source questioned scans and two same-source questioned scans.

\item[VCMER] \citep{vcmer} is an open-set study of breech face comparisons using virtual microscopy. Each of 76 participants (primarily from the US and Canada) received 16 test sets each consisting of two reference scans and one questioned scans. In 17 sets, the questioned bullet matched the references, and in 23 sets, the questioned bullet was fired from a different weapon.

\item[Mattijssen] \citep{mattijssenValidityReliabilityForensic2020} is an open-set study of firing pin aperture mark images and scans from 200 Glock pistols. Sixty of the resulting pairwise comparisons were shown to 77 firearms examiners, who provided assessments based on a likelihood-ratio scale initially, and were then asked whether they would have returned an inconclusive decision after the fact. The results from the study were reported by the authors in both AFTE and LR formats, which allows us to include it in this assessment. A large majority (58) of the participants in this study report case work conclusions categorically (e.g.\ according to AFTE or similar scales).

\item[FAID-2009] \citep{pauw2013faid} is an open-set study of bullets and cartridge cases using castings. Each of 64 participants evaluated 10 test sets (5 bullets, 5 cartridge cases). Each test set consisted of three reference castings and 1 questioned casting. In three of the  bullet sets and two of the cartridge case sets, the questioned material matched the references. The examiners  in this study are primarily from Europe, and the study used a 5-class categorical scale. Note that this study was part of a proficiency test, and as such, examiners may have faced different pressures than in the other error-rate studies.
\end{description}
...
These types of studies are not easily comparable to the black-box studies under consideration here: none of the automatic matching algorithms employs a category of `inconclusive' as a result, so the discussions on how to deal with inconclusive results are not applicable.  \svp{This} implies that for algorithms we cannot distinguish between errors stemming from insufficient markings on evidence and errors inherent to the algorithm. \svp{We have included algorithmic results from \citet{mattijssenValidityReliabilityForensic2020}, which included algorithmic analysis as well as examiner evaluations in the appendix for comparison purposes, but will defer additional examination of algorithmic studies to a future paper.}
...
Shortly after the report was issued, additional studies were published which also meet the criteria for reliable studies in the report:  \citet{keisler}, \citet{Duez:2017kha}, \svp{VCMER \citep{vcmer}, and \citet{mattijssenValidityReliabilityForensic2020}}. 
All four ``good" studies used slightly different designs but share one essential element: each kit contained multiple comparisons composed of one or more samples from exactly one known source and one unknown source.
...
All studies with reliable designs are also limited to cartridge case comparisons (one exception to that is FAID09  \citep{pauw2013faid}, which was published before the release of the PCAST report, but is a European-focused study). 
We could not identify any studies that assess the error rates of bullet or toolmark examination in a manner that would produce reliable error rate estimates. \footnote{We have included FAID09 \citep{pauw2013faid}, which examines bullets and cartridge cases, but uses European examiners (who are trained to different standards and some of whom work in non-adversarial systems), and a 5-point categorical range of conclusions. 
For these reasons, this study is useful but is not necessarily a good representation of studies using the AFTE scale with US and Canadian examiners.}
...
The Bunch study, for instance, was conducted at the FBI, where all examiners followed the same lab policy precluding eliminations based on class characteristic mismatches; as a result, the study has the largest difference between the two missed elimination rates.

Part of this difference can be attributed to lab policies: in some labs exclusions based on individual characteristics are not allowed (i.e.\ class characteristics match). However, the effect of lab policies does not explain all: 
 \citet{Baldwin:2014bb} made efforts to control for the effects of lab policy in determination of inconclusive findings, the efforts were not completely successful.
...
\svp{It is apparent that there is a distinct difference between the error rates and inconclusive rates in US/CA and EU-focused experiments, showing the clear impact of the confluence of training, legal system, reporting scale, and the type of error rate study (proficiency exam vs. error rate assessment).}
...
More realistic estimates of error rates that account for the use of deductive reasoning can be found in \Cref{sec:sim}.


<<errors, echo=FALSE, fig.width=8, fig.height=5.85,fig.cap="Percentages of missed eliminations and missed identifications by study. 95\\% Pearson-Clopper confidence intervals are drawn around the error estimates. Missed eliminations cannot be calculated for closed-set studies.", out.width=".8\\textwidth">>=

errors <- errors %>% 
  mutate(
    Error = factor(Error)
  )
levels(errors$Error) <- c("Missed\nElimination", "Missed\nIdentification")

gg <- errors %>% 
  gather(Type, Number, AFTE, Process, Elimination) %>% 
  mutate(
...
gg <- gg %>% group_by(Type, Study, Training, Error) %>%
  mutate(
...
rects <- errors %>%
  select(Study, Training, Error) %>%
  unique() %>%
  mutate(y = as.numeric(Study)) %>%
  filter(y %in% c(1, 3, 5, 7, 9))

# rects <- crossing(Training = unique(errors$Training), errors = unique(errors$Error), y = c(1, 3, 5, 7, 9)) %>%
  

gg %>%
  ggplot(aes(x = Number/Comparisons*100, y = Study)) +
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin =y - 0.5, ymax = y + 0.5),
            fill = "grey60", alpha = .25, inherit.aes = F, data = rects) +
  geom_point(aes(shape=Type, colour = Type, y = as.numeric(Study)+0.3*(as.numeric(Type)-2)) ) +
  ...
  xlim(c(0, NA)) +
  geom_errorbarh(aes(xmin=err_lower, xmax=err_upper, colour = Type, 
                     y=as.numeric(Study)+0.3*(as.numeric(Type)-2)),
                 size=0.5, height = 0.25,  alpha =0.9) +
  scale_y_continuous(breaks=1:length(levels(errors$Study)), labels=levels(errors$Study), minor_breaks = NULL) +
  scale_colour_brewer(palette="Dark2") +
  theme(legend.position = "bottom", legend.direction = "horizontal") +
    facet_grid(Error+Training~.,scales="free_y", space="free_y")  #, space="free") 
...
It is also apparent that examiners have much more difficulty when examining evidence that arises from different sources.  As an example, examiners who participated in the Bunch study \citep{bunch2003comprehensive} had a significantly higher probability of reaching an inconclusive conclusion than an elimination when evidence items had a different source.
...
Regardless of the explanation, it is apparent that when presented with a different source comparison, examiners make an elimination less frequently than an identification when presented with a same-source comparison. 

<<source-spec-cis, echo=FALSE, fig.width=8, fig.height=5.85, fig.cap="Pearson-Clopper 95\\% confidence intervals for the probability of an examiner's conclusion given source of the evidence. No assessments can be made about different-source specific probabilities when the total number of comparisons cannot be determined. The probability of inconclusive determinations on different source evidence is quite high, much higher than for same source evidence, where the probability for an inconclusive result is close to zero. FAID09 \\citep{pauw2013faid} is unique among the rest of the studies: for both sets, the rate of inconclusive decisions is similar for same and different source evidence.", out.width=".8\\textwidth">>=
...
figdata$Ground.truth <- factor(figdata$Ground.truth)
levels(figdata$Ground.truth) <- c("Diff. Src", "Same Src")

DontKnow_source_spec$Ground.truth <- factor(DontKnow_source_spec$Ground.truth)
levels(DontKnow_source_spec$Ground.truth) <- c("Diff. Src", "Same Src")

figdata %>%
  ggplot(aes(y = pred_ss, x = Study, colour = Decision, shape = Decision)) +
  facet_grid(Ground.truth+Training~., space="free_y", scales="free_y") +
  ylim(c(0,1)) +
  ...
  scale_colour_brewer(palette="Dark2") +
   geom_col(aes(x = Study, y = I(1)), width = 1, colour = NA,
          ...
          data = figdata %>%
 filter(as.numeric(Study) %in% c(1, 3, 5, 7, 9))) 
@
...
<<cis, echo=FALSE, fig.width=8, fig.height=7, fig.cap="Pearson-Clopper 95\\% confidence intervals for the probability of same source evidence given an examiner's conclusion. Expected values for each of the probabilities as discussed in \\Cref{sec:inconclusives} are shown as grey targets. Some of the studies do not allow an assessment of all of the predictive values. \\svp{Note that all 3 sets with non-AFTE conclusions \\citep{pauw2013faid, mattijssenValidityReliabilityForensic2020} have much higher rates of same-source elimination errors. Only FAID09 \\citep{pauw2013faid} has inconclusive rates which are similar to the overall rate of same-source evidence.}", out.width=".8\\textwidth">>=
...
labeltext <- 
  data.frame(x = c(0.9, 0.1, 0.9), y = 8, label="EU",
             Decision=c("Elimination", "Identification",
                        "Inconclusive"))

cis %>%
  ggplot(aes(x = pred_ss, y = Study)) +
  facet_grid(Decision~.) +
  geom_rect(xmin=-Inf, xmax=Inf, ymin=7.5, ymax=Inf, fill="black", alpha=0.01) +
  xlim(c(0,1)) +
...
#  geom_point(aes(x = exp_prob, y = Study), data = exp, colour="white", size=3.5) +
  ...
  geom_text(aes(x=x, y=y, label=label),  colour="grey70", 
            size=10, vjust=-0.15, hjust=0.5, data = labeltext) 

@


In \Cref{fig:cis}, it is  clear that the probability of same-source evidence given an inconclusive evaluation is much lower than the \svp{expected proportion} (target probability) \svp{for most studies}.
...
\svp{One notable exception is FAID09 \citep{pauw2013faid}, which is a proficiency test (and thus, may be harder than the studies focused on error rates) of examiners in various European laboratories. While we cannot determine \emph{why} this study is so different, it is clear that the discrepancy between inconclusive determinations and evidence source does not exist under all conditions.
This suggests we may want to examine the source of this bias in more depth.}
...
This implies that we can calculate the ``target" probability for each decision. \svp{For same-source pairs, } the target for identification is 100\%: all same source pairs should result in an identification. 
...
\begin{table}
\caption{Expected (target) conditional probabilities in a world where examiners do not make mistakes but do make inconclusive decisions. }\label{tab:ideal-probabilities}
{\tabulinesep=1.2mm
\begin{tabu}{|c|ccc|}\hline
$P(X | Y)$ & \multicolumn{3}{c|}{$Y = $Examiner Conclusion} \\\hline
$X = $Ground Truth & Identification & Inconclusive & Elimination \\\hline
...
However, in Baldwin, as in the other studies, the probability of same-source evidence given an inconclusive is lower than the expectation.
...
There is an order of magnitude difference between the probability of same-source evidence given an elimination and the probability of different-source evidence given an identification \svp{in studies conducted using examiners who are primarily from the US and Canada (e.g. excluding \citet{mattijssenValidityReliabilityForensic2020, pauw2013faid}).} 
In fact, even if inconclusives and eliminations are combined as under Option 4 \svp{in \citet{Baldwin:2014bb}}, the probability of a false elimination is 0.0069, only a third of the probability of a false identification.
These observations are shown visually in \Cref{fig:asymmetric-errors} and explicitly calculated in \Cref{eqn:baldwin-calcs} for \citet{Baldwin:2014bb}, \svp{the only study from the US or Canada} with sufficient error counts.
...
\svp{If we repeat these calculations using another large study, \citet{mattijssenValidityReliabilityForensic2020}, however, we find that this issue is not universal.}
...
\svp{While we cannot conclude that this difference is due to the location of the examiners, or the protocol by which the study was conducted, it is clear that the studies conducted primarily on US examiners show a bias that does not exist in \citet{mattijssenValidityReliabilityForensic2020}; that is, we do not have to be content with this bias as the status quo.
More studies are needed to determine which factors (training, location, legal system, system of reporting conclusions, study protocol) are most important in contributing to the clear bias in the distribution of inconclusives across same-source and different-source comparisons.}

<<asymmetric-errors, echo = F, fig.width = 8, fig.height = 4, fig.cap = "Decision-specific error rates. Note that the error percentages are shown on a log scale to allow visualizing differences in errors for both European studies and studies from he US and Canada. The European studies exhibit errors at an order of magnitude higher than US/CA based studies.  For all US/CA studies in which any errors happened,  errors in identification are more likely than errors in eliminations. This difference is significant in \\citet{Baldwin:2014bb},  the only US/CA study with a large number of participants and evaluations. In all but the VCMER (2020) study, counting inconclusives as eliminations does not significantly increase the corresponding error rate, and makes error rates from both DS and SS materials more similar.", cache = F>>=
...
errors2 <- studies %>% 
  ...
  ungroup() %>%
  mutate(
    afte = (Study %in% c("Mattijssen (2020)", "FAID09 (2013) Bull.", "FAID09 (2013) Cartr.")) %>% factor(levels = c(FALSE, TRUE), labels = c("US/CA", "EU")),
    open = rate == 0
  ) %>%
  arrange(Study) %>% 
  mutate(
...
gg2 <- errors2  %>%
  ...
  geom_hline(yintercept = "FAID09 (2013) Bull.", colour = "black",
             size=12, alpha = 0.1) +
  ...
  geom_point(aes(colour = ErrorType, shape = ErrorType),
             ...
             size = 2.5) + 
  geom_errorbarh(
    aes(xmin = lower*100, xmax = upper*100, colour = ErrorType), 
    position = position_dodge(width = 0.6), height = .3) +
  ...
  facet_grid(afte ~ ., scales = "free", space = "free") +
  scale_x_log10(minor_breaks=c(0.01*1:10, 0.1*1:10, 1:10,10*(1:10)))
#  scale_x_log10(minor_breaks=c(0.04, 0.07, 0.4, 0.7, 4, 7, 40, 70))

ggb2 <- ggplot_build(gg2)
ggb2$data[[4]] <- ggb2$data[[4]] %>% 
  mutate(
    shape = ifelse(is.infinite(x) & shape==15, 22, shape),
    shape = ifelse(is.infinite(x) & shape==16, 21, shape),
    shape = ifelse(is.infinite(x) & shape==17, 24, shape),
    size = ifelse(is.infinite(x), 3, size)
  )

gg3 <- ggplot_gtable(ggb2)
plot(gg3)

@


Note that while the probability of same source given an elimination is affected by lab policies towards the treatment of inconclusives, these policies do not explain the continued discrepancy in error probabilities when inconclusives and eliminations are considered together. 
\svp{The fact that all of the AFTE studies and none of the non-AFTE studies display this trend suggests that while lab policies do not explain the discrepancy, the training examiners receive, the scale used to evaluate evidence, and the underlying culture of the legal system (e.g. adversarial vs. non-adversarial) might be factors relevant to this discrepancy. The non-AFTE studies were conducted using worldwide or European participants, who have different training and ongoing evaluation requirements, as well as fundamentally different legal systems. }



% \Cref{fig:baldwin-shape-overview} provides a visual demonstration of this discrepancy: unlike in \Cref{fig:shape-overview}, there is a difference in the density of points in the inconclusive sections of the chart. 
...
\svp{We will ignore \citet{mattijssenValidityReliabilityForensic2020} for the moment, as it was conducted using likelihood ratio based reasoning (with inconclusives as an afterthought) and thus is not completely comparable to studies which use an AFTE-like scale with inconclusives by default.} 
\svp{In addition, we will exclude the Keisler study from consideration here because it did not record any definitive (not-inconclusive) errors; as a result, we cannot precisely estimate the rate of same source eliminations or different-source identifications.}
\svp{In the three remaining studies conducted using the AFTE range of conclusions that have at least one definitive error, } the point estimate of the probability of a false identification is higher than the probability of a false elimination. In the case of \citet{Baldwin:2014bb}, the difference is statistically significant \svp{(Baldwin is the only study which observed definitive false identifications and false eliminations)}.
...
In the case of bullet examination, there is an objective criterion for sufficient similarity: 6 consecutively matching striae, according to \citet{biasotti}; similar criteria have been proposed for algorithmic assessment of cartridge cases \citep{song3DTopographyMeasurements2014}.
...
All of the published \svp{AFTE scale} studies that meet at least some of the design criteria focus only on cartridge cases; it is imperative that similarly well designed studies examining bullets and toolmarks be conducted and published to validate the entire discipline of firearms and toolmark analysis.
...
Examiner-specific error rates should not count inconclusives as errors, while  process-specific error rates should have to count inconclusives as errors.  
Process-specific error rates would then reflect the inability of the examiner to make a correct determination due to insufficient identifying information on the evidence.
...
Specifically, in court, we suggest that when the admissibility of an examiner's testimony is assessed, the examiner is asked to state the lab policy on making eliminations \svp{and} the rate at which the examiner makes inconclusive decisions, along with any relevant error rates specific to the lab and to themselves. We also suggest that during cross-examination after the testimony was presented, the defense ask about the decision-specific probability of an error (relative to the examiner's decision). \svp{This information will provide error rates which are specific and relevant to the presented testimony.}% which will provide specific information which is relevant to the testimony at hand.
...
In the process of examining these different error rate calculations, we discovered a series of systematic biases in error rate studies and the examination process which all work against the defendant. 
For example, in many studies, the \svp{study is designed such that it is possible to estimate false elimination rates} more precisely than false identification rates. 
...
This ensures that there is no way to invalidate the examiner's testimony on the basis that it might contribute to a false conviction of the defendant. 
In addition, examiners \svp{working under the AFTE range of conclusions} appear to have a lower threshold for identification than for elimination; when evidence originates from different sources, examiners are more likely to arrive at an inconclusive decision than they are when the evidence has the same source. 
...
\svp{This paper briefly examines two non-AFTE studies, FAID09 \citep{pauw2013faid} and \citet{mattijssenValidityReliabilityForensic2020}; these studies do not demonstrate this bias, but it is not clear whether the difference is the population (examiners from outside the US and Canada), the scales used by the examiners, the legal systems, or some other factor. 
Additional studies should be performed to investigate possible explanations for this difference and to determine whether the percieved distinction between study types holds up under scruitiny.}
...
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{0.9799} & \multicolumn{1}{c|}{0.0147} & \multicolumn{1}{c|}{0.0028} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.0201} & \multicolumn{1}{c|}{0.9853} & \multicolumn{1}{c|}{0.9972} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{1097} & \multicolumn{1}{c}{748} & \multicolumn{1}{c|}{1425} & \\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 0.9862 & 0.0101 & 0.0037 & 1090\\\cline{2-4}
Different source & 0.0101 & 0.3381 & 0.6518 & 2180\\\hline
\multicolumn{1}{Y}{ } & \multicolumn{4}{l}{\emph{Experiment Count Data are divided by Source Totals (last column)}} \\\hline\hline
\end{tabularx}
...
\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{1.0000} & \multicolumn{1}{c|}{0.0193} & \multicolumn{1}{c|}{0.0000} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.0000} & \multicolumn{1}{c|}{0.9807} & \multicolumn{1}{c|}{1.0000} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{1508} & \multicolumn{1}{c}{207} & \multicolumn{1}{c|}{805} & \\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 0.9974 & 0.0026 & 0.0000 & 1512\\\cline{2-4}
Different source & 0.0000 & 0.2014 & 0.7986 & 1008\\\hline
\multicolumn{1}{Y}{ } & \multicolumn{4}{l}{\emph{Experiment Count Data are divided by Source Totals (last column)}} \\\hline\hline
\end{tabularx}
...
\multicolumn{1}{Y}{Conclusion Total} & 276+61 & 12+14 & \multicolumn{1}{r}{80+5} & \multicolumn{1}{Y}{448}\\\hline\hline

\multicolumn{5}{c}{}\\

\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{0.9941} & \multicolumn{1}{c|}{0.0385} & \multicolumn{1}{c|}{0.0000} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.0059} & \multicolumn{1}{c|}{0.9615} & \multicolumn{1}{c|}{1.0000} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{337} & \multicolumn{1}{c}{26} & \multicolumn{1}{c|}{85} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 0.9970 & 0.0030 & 0.0000 & 336\\\cline{2-4}
Different source & 0.0179 & 0.2232 & 0.7589 & 112\\\hline
\multicolumn{1}{Y}{ } & \multicolumn{4}{l}{\emph{Experiment Count Data are divided by Source Totals (last column)}} \\\hline\hline
\end{tabularx}
...
\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{1.0000} & \multicolumn{1}{c|}{?} & \multicolumn{1}{c|}{?} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.0000} & \multicolumn{1}{c|}{?} & \multicolumn{1}{c|}{?} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{10447} & \multicolumn{1}{c}{?} & \multicolumn{1}{c|}{?} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 0.9992 & 0.0008 & 0.0000 & 10455\\\cline{2-4}
...
4 & Inconcl. = Elim.& 0.0008 & ? & ? \\\hline
\end{tabularx}
\end{fmpage}

\clearpage
\paragraph{Bunch}  \citep{bunch2003comprehensive} used a study design that is not conducive to a quick summary. The study consisted of 8 test kits of varying composition; the test kits were evaluated by 8 examiners at the FBI laboratory. 

\noindent\begin{fmpage}{.985\textwidth}\centering % Checked numbers 2020-04-24
\captionsetup{type=table}\captionof{table}{Results and summary tables for the Bunch study. Values in this table are as reported in the study; however, due to the study's structure, it is possible to perform fewer comparisons, because not all comparisons are independent. Results for independent comparisons are shown in \Cref{tab:bunch-indep}}\label{tab:bunch-summary}
...
\multicolumn{1}{Y}{Conclusion Total} & 70 & 172 & \multicolumn{1}{r}{ 118} & \multicolumn{1}{Y}{360}\\\hline\hline
\multicolumn{5}{c}{}\\

\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{1.0000} & \multicolumn{1}{c|}{0.0000} & \multicolumn{1}{c|}{0.0000} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.0000} & \multicolumn{1}{c|}{1.0000} & \multicolumn{1}{c|}{1.0000} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{70} & \multicolumn{1}{c}{172} & \multicolumn{1}{c|}{118} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 1.0000 & 0.0000 & 0.0000 & 70\\\cline{2-4}
Different source & 0.0000 & 0.5931 & 0.4069 & 290\\\hline
\multicolumn{1}{Y}{ } & \multicolumn{4}{l}{\emph{Experiment Count Data are divided by Source Totals (last column)}} \\\hline\hline
\end{tabularx}
...
\multicolumn{5}{c}{\bf Experiment Count Data}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Total\\\hline
Same Source & 174\footnote{ \citet{lyons} reports 175 correct identifications, but it is clear from the discussion that one of those same source identifications was in fact an inconclusive. Twelve answer sheets with 12 correct identifications, one with 10 (out of 10) correct identifications, one with 9 correct identifications and 3 errors, and one with 11 correct identifications and one inconclusive. So 12$\cdot$12 + 10 + 9 + 11 = 174. } &  1 & 3 & 178 \\
...
\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{0.9831} & \multicolumn{1}{c|}{?} & \multicolumn{1}{c|}{?} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.0169} & \multicolumn{1}{c|}{?} & \multicolumn{1}{c|}{?} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{177} & \multicolumn{1}{c}{?} & \multicolumn{1}{c|}{?} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 0.9775 & 0.0056 & 0.0169 & 178\\\cline{2-4}
Different source & ? & ? & ? & ?\\\hline
\multicolumn{1}{Y}{ } & \multicolumn{4}{l}{\emph{Experiment Count Data are divided by Source Totals (last column)}} \\\hline\hline
\end{tabularx}
...
\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{0.9934} & \multicolumn{1}{c|}{0.1301} & \multicolumn{1}{c|}{0.0000} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.0066} & \multicolumn{1}{c|}{0.8699} & \multicolumn{1}{c|}{1.0000} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{456} & \multicolumn{1}{c}{292} & \multicolumn{1}{c|}{436} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 0.9226 & 0.0774 & 0.0000 & 491\\\cline{2-4}
Different source & 0.0043 & 0.3665 & 0.6291 & 693\\\hline
\multicolumn{1}{Y}{ } & \multicolumn{4}{l}{\emph{Experiment Count Data are divided by Source Totals (last column)}} \\\hline\hline
\end{tabularx}
...
\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{0.9697} & \multicolumn{1}{c|}{0.3740} & \multicolumn{1}{c|}{0.1081} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.0303} & \multicolumn{1}{c|}{0.6260} & \multicolumn{1}{c|}{0.8919} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{2439} & \multicolumn{1}{c}{1302} & \multicolumn{1}{c|}{879} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 0.8025 & 0.1653 & 0.0322 & 2947\\\cline{2-4}
Different source & 0.0442 & 0.4871 & 0.4686 & 1673\\\hline
\multicolumn{1}{Y}{ } & \multicolumn{4}{l}{\emph{Experiment Count Data are divided by Source Totals (last column)}} \\\hline\hline
\end{tabularx}
...
\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{0.1636} & \multicolumn{1}{c|}{-} & \multicolumn{1}{c|}{0.0000} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.8364} & \multicolumn{1}{c|}{-} & \multicolumn{1}{c|}{1.0000} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{1210} & \multicolumn{1}{c}{-} & \multicolumn{1}{c|}{78590} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 0.9900 & - & 0.0100 & 200\\\cline{2-4}
Different source & 0.0127 & - & 0.9873 & 79600\\\hline
\multicolumn{1}{Y}{ } & \multicolumn{4}{l}{\emph{Experiment Count Data are divided by Source Totals (last column)}} \\\hline\hline
\end{tabularx}
...
\multicolumn{1}{Y}{Conclusion Total} & 1197 & 0 & \multicolumn{1}{r}{78603} & \multicolumn{1}{Y}{79800}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{0.1654} & \multicolumn{1}{c|}{-} & \multicolumn{1}{c|}{0.0000} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.8346} & \multicolumn{1}{c|}{-} & \multicolumn{1}{c|}{1.0000} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{1197} & \multicolumn{1}{c}{-} & \multicolumn{1}{c|}{78603} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 0.9900 & - & 0.0100 & 200\\\cline{2-4}
Different source & 0.0126 & - & 0.9874 & 79600\\\hline
\multicolumn{1}{Y}{ } & \multicolumn{4}{l}{\emph{Experiment Count Data are divided by Source Totals (last column)}} \\\hline\hline
\end{tabularx}
...
Different source & 5 & 47 & 72 & 124  \\\hline
\multicolumn{1}{Y}{Conclusion Total} & 88 & 146 & \multicolumn{1}{r}{78} & \multicolumn{1}{Y}{312}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{0.9432} & \multicolumn{1}{c|}{0.6781} & \multicolumn{1}{c|}{0.0769} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.0568} & \multicolumn{1}{c|}{0.3219} & \multicolumn{1}{c|}{0.9231} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{88} & \multicolumn{1}{c}{146} & \multicolumn{1}{c|}{78} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 0.4415 & 0.5266 & 0.0319 & 188\\\cline{2-4}
Different source & 0.0403 & 0.3790 & 0.5806 & 124\\\hline
\multicolumn{1}{Y}{ } & \multicolumn{4}{l}{\emph{Experiment Count Data are divided by Source Totals (last column)}} \\\hline\hline
\end{tabularx}
...
\noindent\begin{fmpage}{.985\textwidth}\centering % Checked numbers 2020-04-24
\captionsetup{type=table}\captionof{table}{FAID09 study results for cartridges, conclusion-specific and source-specific probabilities, and reported overall error rates. }\label{tab:FAID09-cartridge-summary}
{\large\bf FAID2009 Proficiency Test and Workshop}
\emph{ \citeauthor{pauw2013faid} ( \citeyear{pauw2013faid})}\\
\begin{tabularx}{\textwidth}{XrrX}\hline
\multirow{2}{*}{\bf Study Type} & \multicolumn{2}{c}{\bf Test Set} & \multirow{2}{*}{\bf Participants}\\
...
Different source & 15 & 55 & 119 & 189  \\\hline
\multicolumn{1}{Y}{Conclusion Total} & 113 & 77 & \multicolumn{1}{r}{126} & \multicolumn{1}{Y}{316}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{c}{Elimination} & \\\cline{1-4}
Same source & \multicolumn{1}{c|}{0.8673} & \multicolumn{1}{c|}{0.2857} & \multicolumn{1}{c|}{0.0556} & \multirow{3}{=}{\emph{Experiment Count Data are divided by Conclusion Totals (3rd row)}}\\
Different source & \multicolumn{1}{c|}{0.1327} & \multicolumn{1}{c|}{0.7143} & \multicolumn{1}{c|}{0.9444} &\\\cline{1-4}
Total \# Comparisons & \multicolumn{1}{c}{113} & \multicolumn{1}{c}{77} & \multicolumn{1}{c|}{126} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Total \# Comparisons\\\hline
Same source & 0.7717 & 0.1732 & 0.0551 & 127\\\cline{2-4}
Different source & 0.0794 & 0.2910 & 0.6296 & 189\\\hline
\multicolumn{1}{Y}{ } & \multicolumn{4}{l}{\emph{Experiment Count Data are divided by Source Totals (last column)}} \\\hline\hline
\end{tabularx}
...
\section{Visual Examination of Experimental Frequencies}\label{sec:exp-freq}
This section shows visual equivalents of \Cref{fig:shape-overview}, but for these studies, there is generally a very clear difference between the probability of inconclusive decisions under different and same source comparisons.

<<shape-overview-study, fig.cap = "Sketch of the relationship between  ground truth of evidence (dots) and examiners' decisions (shaded areas). In \\citet{mattijssenValidityReliabilityForensic2020}, each dot represents approximately 4 examiner evaluations; in \\citet{Baldwin:2014bb} each dot represents 2 examiner evaluations.", fig.width = 12, fig.height = 8, dpi = 1000, out.width = "\\textwidth", cache = F>>=
source("code/make_inconcl_plot.R")
mattijssen <- as.list(round(c(a = 2365, b = 487, c = 95, d = 74, e = 815, f = 784)/4))
lilien <- list(a = 453, b = 38, c = 0, d = 3, e = 254, f = 436)
baldwin <- as.list(round(c(a = 1075, b = 11, c = 4, d = 22, e = 737, f = 1421)/2))
keisler <- list(a = 1508, b = 4, c = 0, d = 0, e = 203, f = 805)
duez <- list(a = 276+59, b = 1, c = 0, d = 2, e = 25, f = 85)
bunch <- list(a = 70, b = 0, c = 0, d = 0, e = 172, f = 118)

grid.arrange(
do.call(target_plot, c(mattijssen, pointsize = .5, bordersize = 0.001, title = "Mattijssen (2020)", legend = F)),
do.call(target_plot, c(lilien, pointsize = .5, bordersize = 0.001, title = "VCMER (2020)", legend = F)),
do.call(target_plot, c(baldwin, pointsize = .5, bordersize = 0.001, title = "Baldwin (2014)", legend = F)),
do.call(target_plot, c(keisler, pointsize = .5, bordersize = 0.001, title = "Keisler (2018)", legend = F)),
do.call(target_plot, c(duez, pointsize = .5, bordersize = 0.001, title = "Duez (2018)", legend = F)),
ncol = 3
)
@
...
\subsection{Bunch simulation}\label{sec:bunchmurphysim}
