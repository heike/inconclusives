...
\svp{
The response identifies only one study\cite{Baldwin:2014bb} as appropriately designed to evaluate the validity and reliability of firearms analysis methods. 
Since that response was issued in \citeyear{pcast-addendum2017}, there have been two more studies which are also well-designed according to the criteria in the report: \cite{keisler} and \cite{Duez:2017kha}. 
...
\svp{
Study designs that were less reliable, such as \cite{lyons}, \cite{Hamby:2018hu} (and the similarly designed \cite{fadul}), and \cite{bunch2003comprehensive}. 
Of these, the study by \citeauthor{bunch2003comprehensive} is perhaps the least flawed: while it has internal dependencies due to the inclusion of multiple knowns, and while the number of same-source and different-source comparisons cannot be fully determined, it does have one feature not found in even the well designed studies.
...
XXX Why hamby/fadul suck, and why lyons sucks even more.
}
