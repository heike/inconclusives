...
\begin{itemize}\item A given test has a sensitivity (true positive rate, TPR) and a specificity (true negative rate, TNR) that combined determine the overall accuracy rate. \svp{These quantities are useful for an accrediting agency or company, which is interested in whether the test works as claimed.} 
\item \svp{A patient who has received a negative test result is more interested in a different value: the probability that we actually have the disease 
%(and could be contagious) 
considering that the patient has received a negative test result.} \svp{At the individual level, we want to use tailored information to guide our specific actions; at the system level, we are more interested in gross measures of performance.}
\end{itemize}
