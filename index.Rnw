...
It has now been more than a decade since the publication of the \citet{nrc:2009} report, which discussed the need for better designed studies of error rates in forensic disciplines. 
...
For each study, we calculate the error rates from the study results using standardized methods. 
We also assess the impact of study design and treatment of inconclusives on the calculated error rates.
In order to lay a foundation for the common calculation methods used in this paper, we will first introduce some theory and vocabulary before comparing the studies and assessing the state of error rates in firearms and toolmark analysis.
...
However, open set studies provide only limited additional information even in the case where  multiple unknown sources are compared to a single known source. %, and only in the case where two or more of the unknown sources match each other.
...
\hh{Making a decision on evidence as identification, exclusion or inconclusive provides a \emph{classification} of the evidence, which allows us to make use of elements of the larger \emph{classification framework}.}
...
An alternate way to evaluate a classification method is to assess the probability of a same-source or diferent-source comparison, given an examiner has  made a  decision (or, more generically, a specific general conclusion).
...
\end{itemize}
% \hh{Similarly, we might also interested in the false negative rate, i.e. if we, 
 % as patients receive a negative test result, what is the probability that, given this negative result, we actually do have the disease (and could be contagious)?}
% This probability is the false omission rate, or FOR of the test, and is equivalent to the decision-specific probability $P(SS\mid \text{Exclusion})$. 
...
There are several promising automatic analysis tools \citep[][see]{aoas2,case-validation,chumbleyValidationToolMark2010,chuAutomaticIdentificationBullet2013,songEstimatingErrorRates2018,tai2018fully} which can generate error rates from scans of bullet and cartridge cases, but these types of error rates are not easily comparable to error rates derived from black-box studies, if only because algorithmic results will be the same if repeated on a different date. 
% Two other important studies, \citet{aoas2} and \citet{chumbleyValidationToolMark2010}, have also been excluded from this analysis. \citet{aoas2} presents a quantitative method for comparison of bullets (but has no experimental lab based examiner assessments), and \citep{chumbleyValidationToolMark2010} compares a computational method for bullet analysis to examiner evaluations, but does not provide the results of the examiner study in sufficient detail for our purposes. 
% \hh{Besides the seven studies identified here, ... XXX?}
% \svp{
% Of the seven studies identified, one  \citep{aoas2} presents a method for quantitative comparison of bullets with no experimental lab-based examiner assessments. 
...
Of these, the study by  \citeauthor{bunch2003comprehensive} is perhaps statistically the most interesting design, but it has been superceded by studies with cleaner experimental designs, such as \citet{Baldwin:2014bb} and \citet{keisler}.
While \citet{bunch2003comprehensive} has internal dependencies due to the inclusion of multiple knowns, and as a result, the number of same-source and different-source comparisons cannot be fully determined, the study does have one desireable feature not found in even the well designed studies which makes it worth discussing here.
...
In both the multiple known and closed set study designs, the use of deductive reasoning artificially reduces the error rates calculated using the study results, as discussed in the PCAST report and addendum \citep{pcast2016,pcast-addendum2017}. %\hh{XXX that has been pointed out in the PCAST report XXX}
...
%\svp{XXX not sure this is true - need to think through it...}
...
However, in \citet{Baldwin:2014bb}, it is clear that examiners are willing to gamble on an indentification much more than they are willing to gamble on an elimination. 
In the courtroom (or before, in a plea bargin situation), this results in a clear bias toward the prosecution (or against the defense).
