...
\svp{While these two statements seem to be contradictory, in this paper, we provide a foundation under which the two approaches can be partially reconciled in a way that provides additional insight in to the examination process.} % my attempt
...
\autoref{tab:overall-err-rate} shows an overview of these three approaches to calculating the error rate based on the general structure of \autoref{tab:generic-results}, \svp{with a fourth option which will be addressed in more detail in \autoref{sec:inconclusives}}. 
...
\svp{In option 4, which will become more relevant in \autoref{sec:inconclusives}, inconclusives are treated the same as eliminations. 
...
%
In the next two sections we introduce two \svp{additional types} of error rates: {\it source-specific} and {\it decision-specific} error rates.
...
\svp{These conditional probabilities provide more specific information about certain scenarios, but do not broadly summarize an experiment using a single number.}
...
\svp{However, some of these} conditional probabilities have the advantage of not requiring inconclusives to be explicitly handled as errors or correct decisions. 
