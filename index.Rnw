...
\hh{Note that we are keeping the definition of error rates a bit vague on purpose. We will pick up the discussion of how to precisely define error rates in~\autoref{error-rates}.}
...
In all  of these assessments, the rates are calculated by conditioning on the origin of the evidence; this process is shown graphically in \autoref{fig:FPR-specific-tree} using a decision tree to describe the comparisons which can be made conditioned on ground truth. 
...
\hh{XXX should we pull up some of the discussion that only regards these error rates? source-specific probabilities can not be calculated from casework, they allow an assessment of the performance of an examiner, but not an assessment of the evidence. source-specific probabilities are the error rates typically reported. Our argument here is that they are largely irrelevant in legal proceedings}
...
An alternate way to evaluate a classification method is to assess \svp{the probability that given an examiner's decision, a certain state of ground truth exists.} \svp{XXX this is awkward... fix}
...
Its complement, the \emph{false omission rate} (FOR) is the probability that, if an examiner makes an elimination, the evidence is actually from the same source.
\footnote{In  \citet{songEstimatingErrorRates2018}, the FDR is referred to as the false identification error rate, and the FOR is called the false exclusion error rate.}
...
Predictive value assessments are used in assessing the implications of a particular classification label; they inform us about the likely state of the physical evidence. 
\svp{This makes the PPV, NPV, FDR, and FOR more appropriate for use when evaluating casework, where the true state of the evidence is unknown.
While mathematically it makes sense to condition on unknown quantities, in practice it is much more informative to condition on quantities whose values are known.}
\svp{For the same reason, the source-specific assessments may be useful when evaluating the performance of individuals on a designed test, because ground truth is known (and thus, can be conditioned upon) in designed studies.}
...
The source-specific error rates,FPR and FNR, are calculated by conditioning on the source of of the evidence (same-source/different source) and give the probability for an examiner's conclusion.
...
From a practical point of view, the question of whether source-specific or decision-specific rates are of interest, is where in the overall (legal) decision process we are: 
\begin{itemize}
...
\item as part of courtroom proceedings, we -- the jury, lawyers, and judges -- are interested in assessing accuracy of testimonies, i.e. we are interested in conclusion-specific rates to answer questions of the form: given an expert testified to the evidence as being an identification, what is the probability that the evidence actually does come from the same source? \svp{In this situation, we can update the overall error rate by conditioning on the examiner's conclusion, providing more specific evidence about the scenario under investigation. As we only know the examiner's testimony (and not ground truth), we must work with the conclusion-specific rates. }
\end{itemize}
...
As we rely on these designed studies to generalize to casework, it is of great importance that the studies be well designed so as to allow for the estimation of any of the success or error rates discussed above.
}
...
\section{Assessment of error rates in case studies}\label{sec:inconclusive}

In order to calculate the two sets of conditional probabilities, 
...
\end{tabularx}
\end{table}
A google sheets workbook which will perform all of these calculations when the frequency of each type of comparison is entered is available at \url{http://bit.ly/FTE-error-rate-worksheet}.
...
\svp{XXX this transition is a bit rough. Might be worth a bit of a preview between inconclusive and ``ideal" study probabilities, because the connection isn't very obvious from this point in the paper... XXX}
...
\item conclusion-specific error rates
\item inconclusives
\end{enumerate}
...
\subsection{Conclusion-specific error rates}
...
\subsection{Inconclusives}\label{sec:inconclusives}
