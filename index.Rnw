...
By ground truth we mean the knowledge of whether two samples come from the same source or two different sources. Ground truth in studies is only available to the individual setting up the test but not to any of the participants. 
In casework ground truth is not known, it therefore does not allow for an assessment of error rates of the identification process.
...
\svp{Using this framework, an examiner's default decision is inconclusive, which can be altered to identification or elimination given sufficient evidence in either direction.} %\hh{XXX what is the consequence of that? }
\svp{This approach treats inconclusives as neither errors nor correct decisions, but implies that in the presence of a certain amount of data, an inconclusive is not an allowable decision and the examiner must make an identification or elimination.}
This view has drawn some criticism \citep{Biedermann:2018hr,biedermannAreInconclusiveDecisions2019}. 

\hh{XXX should we cite a paper for each one of these decisions?}\svp{I don't know if we could find a paper that does each of these. I'm not aware of any that actually use the "incorrect" option, and I don't think I've seen even the conditional option used.}
...
examining previous error rate studies to assess the technical accuracy and practical utility of the Overall Error Rates. 
Before approaching the actual studies, we will define some basic notation to facilitate comparison of studies with different experimental designs and briefly explore the logic behind the numerical calculations.%order
...
The most common way to assess the accuracy of a classification method is to calculate its \emph{sensitivity} and \emph{specificity}. 
...
Another common method of assessing accuracy of a classification method is to calculate the false positive rate (FPR) and the false negative rate (FNR). The \emph{false positive rate} is the probability that an examiner makes an identification from evidence which is from different sources(DS), $FPR = P(\text{Identification} | DS)$.
...
\caption{Tree Diagram illustrating the logic of source-specific error rates. Source-specific rates give the probability of an examiner's decision ( Identification, Inconclusive, or Elimination) given same-source and different-source evidence.}\label{fig:FPR-specific-tree}
\end{figure}
...
Note that the definition of blind studies as used above is different from the definitions of a blind study used in \citet{bunch2003comprehensive} or \citet{Duez:2017kha}; in those papers, the use of the terms blind and/or double blind do not conform to the generally accepted definitions. \citet{kerkhoffPartdeclaredBlindTesting2018} provides a more thorough discussion of the different ways ``blind testing" has been used in forensic error rate studies.
...
\end{tabularx}
\end{table}
A google sheets workbook for performing these calculations is available at \url{http://bit.ly/FTE-error-rate-worksheet}.
...
\svp{
Here, we summarize the primary findings from our survey of the available literature; while we did not attempt to include every study ever performed, we did aim to include studies which are frequently cited in research or legal proceedings, as well as all studies mentioned in the PCAST and NAS reports concerning firearm and toolmark analysis. XXX make sure that's actually true }
...
\svp{ 
Tables describing the results of each of the studies surveyed in this paper, the source-specific and conclusion-specific error rates, and the overall error rates can be found in \ref{sec:supplement}. 
}
...
Two other studies \citep{chumbleyValidationToolMark2010, fadul} did not provide sufficient detail about examiner conclusions to conduct even a partial analysis.
}
...
We also perform a similar assessment for \cite{Hamby:2018hu}; the design of this study does not include the variability introduced by the test kit assembly procedure used in \cite{bunch2003comprehensive}. 
...
\subsection{Overall Error Rates}

\svp{
\autoref{fig:errors} shows the overall error rates for the six studies of examiner error rates investigated in this paper. 
...
Process errors, however, occur at much higher rates and are particularly concerning for missed eliminations. There is consistently a much larger discrepancy between the two error rates for eliminations than for identifications; this phenomena is examined in more detail in \autoref{sec:inconclusives}.
...
The rates reported in this section are calculated directly from information reported in the papers, with no imputation; as such, they represent the best possible case for error rates. 
Less optimistic estimates of error rates that account for the use of deductive reasoning can be found in \ref{app:sim}.
}
...
source_spec_cis %>% 
  ...
  ggplot(aes(y = pred_ss, x = Study, colour = Decision)) +
...
<<inconclusives, echo=FALSE, fig.width=8, fig.height=3.75,fig.cap="Percentage of inconlusive decisions by same-source/different source comparisons. The percentage of inconclusives for same source comparisons is generally low (with a 95\\% confidence interval well below 5\\%). Different source comparisons end up in inconclusive results significantly more often. The Baldwin study has a particularly high percentage of inconclusive results", out.width=".8\\textwidth">>=
...
inc <- studies %>% group_by(Study, Ground.truth) %>% 
  ...
  summarize(
    ...
    inc_lower = clopper(0.05, Inconclusive, Comparisons)[1]*100,
    inc_upper = clopper(0.05, Inconclusive, Comparisons)[2]*100
  )
...
inc %>% 
  ggplot(aes(x = Inconclusive/Comparisons*100, y = Study)) +
  geom_errorbarh(aes(xmin = inc_lower, xmax=inc_upper, y=Study),
                 size=0.5, height = 0.5, colour = "grey20", alpha =0.9) +
  geom_point() +
  theme_bw() +
  facet_grid(Ground.truth~.) +
  xlab("Percentage of inconclusive decisions") +
  geom_errorbarh(aes(x = 0, xmin=xmin, xmax=xmax, y=Study),
                 data = DontKnow, linetype=2,
                 size=0.5, height = 0.5, colour = "grey20", alpha =0.9) +
  geom_point(x = 17.5,  data = DontKnow, shape='?', size = 3.5)
@

\svp{XXX Looking at the predictive value of an inconclusive result led us to the following unexpected (and ugly) realization: Examiners are willing to gamble on an indentification much more than they are willing to gamble on an elimination. This is a clear bias toward the prosecution.}
\svp{XXX Inconclusives bias}
...
``Some [participants] indicated that the design of our study with all cartridges fired from the same model of firearm using the same type of ammunition would prohibit the use of a finding of elimination, while others used a mixture of inconclusive and elimination or did not use inconclusive at all to indicate a finding other than identification. "
...
\begin{itemize}
\item The error rate of certified examiners in making eliminations is much lower than that of the ten trainees. This is reflected in the error rates of the overall process. The process errors are much smaller for certified firearms examiners than for trainees. 
...
218 active firearms examiners participated in the study. Results are summarized in \autoref{tab:baldwin-summary}. 
...
Different source & 22 & 735+2\footnotemark & 1421 & 2180\\\hline
...
\end{tabularx}
\end{fmpage}
\footnotetext{Two comparisons were not reported and are considered to be inconclusives.}

\subsection{Keisler Study}
\svp{The Keisler Study \cite{keisler} was designed so that} each test kit consisted of sets of 20 pairs of cartridge cases from Smith \& Wesson pistols, where 12 of the pairs were from the same source and 8 pairs were from different sources. 
...
126 participants reported results as shown in \autoref{tab:keisler-summary}.
...
\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Probability\\\hline
Same source & 1.0000 & 0.0197 & 0.0000 & 0.6000\\
Different source & 0.0000 & 0.9803 & 1.0000 & 0.4000\\\hline
\multicolumn{1}{Y}{Conclusion Probability} & 0.5984 & 0.0821 & \multicolumn{1}{r}{0.3194} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Probability\\\hline
Same source & 0.9974 & 0.0028 & 0.0000 & 0.6000\\
Different source & 0.0000 & 0.2014 & 0.7976 & 0.4000 \\\hline
\multicolumn{1}{Y}{Conclusion Probability} & 0.5984 & 0.0821 & \multicolumn{1}{r}{0.3194} & \multicolumn{1}{Y}{}\\\hline\hline
\end{tabularx}
...
\subsection{Duez Study}

\citet{Duez:2017kha} used virtual microscopy to evaluate scans of cartridge cases. Each participant was asked to make eight evaluations of breech face impressions. These consisted of two sets: 
\begin{itemize}
...
\multicolumn{5}{c}{\bf Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Probability\\\hline
Same source & 0.9941 & 0.0385 & 0.0000 & 0.7483 \\
Different source & 0.0059 & 0.9615 & 1.0000 & 0.2517 \\\hline
\multicolumn{1}{Y}{Conclusion Probability} & 0.7506 & 0.0579 & \multicolumn{1}{r}{0.1915} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Probability\\\hline
Same source & 0.9970 & 0.0030 & 0.0000 & 0.7483\\
Different source & 0.0179 & 0.2232 & 0.7589 & 0.2517\\\hline
\multicolumn{1}{Y}{Conclusion Probability} & 0.7506 & 0.0579 & \multicolumn{1}{r}{0.1915} & \multicolumn{1}{Y}{}\\\hline\hline
\end{tabularx}
...
\multicolumn{5}{c}{\bf Overall Error Rates}\\
Opt. & \multicolumn{1}{c}{Meaning} & Missed Identification & \multicolumn{1}{r}{Missed Elimination} & Total \\\hline
2 & FTE error & 0.0000/0.0000  & 0.0000/0.1000 & 0.0000/0.0125\\
3 & Process error & 0.0000/0.0167 & 0.1304/0.7500 & 0.0325/0.2000\\ \hline
\end{tabularx}
\end{fmpage}

\subsection{Brundage-Hamby Study}
\subsubsection{Results}

The Brundage-Hamby study consists of sets of 20 test fires from known barrels and 15 questioned bullets. The 20 known test fires are 2 bullets from each of ten consecutively manufactured barrels. The Brundage-Hamby study is a closed set study, i.e. the 15 questioned bullets are known to be fired from one of these ten barrels. Participants (firearm examiners) are asked to identify which of the knowns a questioned bullet matches. 
...
The design of the Brundage-Hamby is well known in the forensics community and has  been copied in \citet{fadul} for a study of cartridge cases of the same firearms.
...
Different source & 0 & ? & ? & 47047.5\footnotemark \\\hline
...
Different source & 0.0000 & ? & 1\footnotemark & 0.8282\\\hline
...
\end{tabularx}
\end{fmpage}
\footnotetext{This number is imputed based on the average number of pairwise comparisons an examiner would have to do to complete a Hamby study. Details can be found in the supplemental material.}
\footnotetext{The elimination-specific conditional probabilities are only identifiable in this case because there were not any false eliminations. In any other situation, we would not be able to calculate these probabilities because we do not have the total number of eliminations.}

\subsubsection{Enumeration of all possible Brundage-Hamby set comparisons}\label{app:brundage}
...
\multicolumn{1}{Y}{Conclusion Probability} & 0.2414 & 0.5931 & \multicolumn{1}{r}{0.4059} & \multicolumn{1}{Y}{}\\\hline\hline
...
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Total\\\hline
Same Source & 19.76 [17, 22] & 0 & 0 & 19.76 [17, 22] \\
Diff Source & 0 & ?? & ?? & 290 \\\hline
\multicolumn{1}{Y}{Conclusion Total} & 70 & 172 & \multicolumn{1}{r}{ 118} & \multicolumn{1}{Y}{}\\\hline\hline
\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Independent Comparisons: Conclusion-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Probability\\\hline
Same source & 1 & 0 & 0 & 0.1109 [0.0256, 0.2381]\\
Diff source & 0 & 1 & 1 & 0.8891 [0.7619, 0.9744]\\\hline
            \multicolumn{1}{Y}{Conclusion Probability} & 0.2414 & 0.5931 & \multicolumn{1}{r}{0.4059} & \multicolumn{1}{Y}{}\\\hline\hline

\multicolumn{5}{c}{}\\
\multicolumn{5}{c}{\bf Independent Comparisons: Source-Specific Probabilities}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Probability\\\hline
Same source & 1.0000 & 0.0000 & 0.0000 & 0.1109 [0.0256, 0.2381]\\
Diff source & 0.0000 & ?? & ?? & 0.8891 [0.7619, 0.9744]\\\hline
\multicolumn{1}{Y}{Conclusion Probability} & 0.2414 & 0.5931 & \multicolumn{1}{r}{0.4059} & \multicolumn{1}{Y}{}\\\hline\hline
\end{tabularx}
...
Opt. & \multicolumn{1}{c}{Meaning} & Missed Identification & \multicolumn{1}{r}{Missed Elimination} & Total \\\hline
2 & FTE error & 0.0000 & 0.0000 & 0.0000\\
3 & Process error & 0.0000 & ?? & ?? \\
\end{tabularx}
\end{fmpage}

\subsection{Lyons Study}

This study involved the use of 10 consecutively manufactured extractors (manufactureed by Caspian Arms Ltd).
...
\footnotetext{\citet{lyons} reports 175 correct identifications, but it is clear from the discussion that one of those same source identifications was actually an inconclusive. 12 answer sheets with 12 correct identifications, one with 10 (out of 10) correct identifications, one with 9 correct identifications and 3 errors, and one with 11 correct identifications and one inconclusive. So 12$\cdot$12 + 10 + 9 + 11 = 174. }
