...
Less optimistic estimates of error rates that account for the use of deductive reasoning can be found in \ref{sec:sim}.
}
...
\svp{From these simulated sets, we can identify any which match the reported values of 70 same-source and 290 possible different-source comparisons. Using these sets, we can then estimate the minimal number of comparisons which are necessary using deductive reasoning in addition to examination.
XXX fill in the details of the deductive reasoning
}
...
Diff Source & 0 & ?? & ?? & 154.22 [139, 171] \\\hline
\multicolumn{1}{Y}{Conclusion Total} & 70 & 172 & \multicolumn{1}{r}{ 118} & \multicolumn{1}{Y}{}\\\hline\hline
\multicolumn{5}{c}{}\\
