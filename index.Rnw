...
In order to calculate the two sets of conditional probabilities, let us assume that we have the results from a generic experiment, where participants were asked to complete a total of $N$ comparisons, consisting of $S$ same-source comparisons and $D$ different source comparisons.
...
in the form of a %table \svp{which can be arranged in a form like the} 
 table as shown in \autoref{tab:generic-results}.

\begin{figure}
\centering
\includegraphics[width=.5\textwidth]{figures/shape-overview.pdf}
\caption{\label{fig:shape-overview} some text.}
\end{figure}

\begin{table}
\centering
\caption{
An example results table for a generic experiment where comparisons can be broken down as either from the same source or from different sources, and examiners classify each comparison as an identification, an inconclusive, or an elimination, as  specified in the AFTE rules of identification. 
...
Then the probability of the occurrence of any cell in the interior of the table $(a, b, c, d, e, \text{ or } f)$ can be found by dividing the respective letter by $N$, the total number of comparisons.
...
Diff Source & \begin{minipage}[c]{1em}$\bm{d}$ \end{minipage}\begin{minipage}[c]{1em} \includegraphics[width=.25cm]{figures/d.pdf} \end{minipage}& 
...
Diff Source & 22 & 735+2\footnotemark & 1421 & 2180\\\hline
...
Diff source & 0.0201 & 0.9853 & 0.9972 & 0.6667\\\hline
...
Diff source & 0.0201 & 0.9853 & 0.9923 & 0.6667\\\hline
\multicolumn{1}{Y}{Conclusion Probability} & 0.3355 & 0.0101 & \multicolumn{1}{r}{0.4379} & \multicolumn{1}{Y}{}\\\hline\hline
\end{tabularx}
...
Each test kit consisted of sets of 20 pairs of cartridge cases, where 12 of the pairs were from the same source and 8 pairs were from different sources, constructed using 9 Smith \& Wesson pistols. \svp{Participants were instructed to only compare single pairs.}
126 participants reported results as shown in \autoref{tab:keisler-summary}.

\noindent\begin{fmpage}{\textwidth}\centering
\captionof{table}{{\large\bf Isolated Pairs Research Study}}
...
\multicolumn{5}{c}{\bf Experiment Count Data}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Total\\\hline
Same source & 1508 & 4 & 0.0000 & 1512 \\
Diff source & 0.0000 & 203 & 805 & 1008 \\\hline
...
Diff source & 0.0000 & 0.9803 & 1.0000 & 0.4000\\\hline
...
Diff source & 0.0000 & 0.2014 & 0..7976 & 0.4000 \\\hline
\multicolumn{1}{Y}{Conclusion Probability} & 0.5984 & 0.0821 & \multicolumn{1}{r}{0.3194} & \multicolumn{1}{Y}{}\\\hline\hline
\end{tabularx}
...
The resulting sets of conditional probabilities are given in the middle two tables of \autoref{tab:keisler-summary}. Again, inconclusive results are much more indicative of different source results than they should be  based on the setup of the study; we would expect same source results with a probability of 0.60 given an inconclusive result. 

\svp{The design of the Keisler study is ideal - it allows for the calculation of all of the error and success rates, all conditional probabilities, and precise identification of the number of same-source and different-source comparisons conducted by each examiner. }
\svp{One major flaw in the Keisler study (that is by no means unique to this study) is the acknowledgement that
...
The top of \autoref{tab:duez-summary} shows results reported by 56 participants (46 examiners, 10 trainees).
...
\multirow{2}{*}{\bf Study Type} & \multicolumn{2}{c}{\bf Test Set} & \multirow{2}{*}{\bf Participants}\\
& \# Same Source Comparisons & \# Different Source Comparisons & \\\hline
Open set & ? & ? & 46+10 examiners\\
\end{tabularx}
\begin{tabularx}{\textwidth}{Y|rrr|Y}
\multicolumn{5}{c}{\bf Experiment Count Data}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Total\\\hline
Same source & 276+59 & 0+1 & 0+0 & 276+60 \\
Diff source & 0+2 & 12+13 & 80+5 & 92+20 \\\hline
\multicolumn{1}{Y}{Concl. Total} & 276+61 & 12+14 & \multicolumn{1}{r}{80+5} & \multicolumn{1}{Y}{449}\\\hline\hline
...
Diff source & 0.0059 & 0.9615 & 1.0000 & 0.2517 \\\hline
...
Diff source & 0.0179 & 0.2232 & 0.7589 & 0.2517\\\hline
\multicolumn{1}{Y}{Conclusion Probability} & 0.7506 & 0.0579 & \multicolumn{1}{r}{0.1915} & \multicolumn{1}{Y}{}\\\hline\hline
\end{tabularx}
...
\autoref{tab:duez-summary} shows the two sets of error rates for the Duez study. The error rate of certified examiners in making eliminations is much lower than that of the ten trainees. This is reflected in the error rates of the overall process. The process errors are much smaller for certified firearms examiners than for trainees. However, the probability to miss eliminations is again high, lab rules contribute to this error (see discussion below).

The set of conditional probabilities are shown at the bottom of \autoref{tab:duez-summary}. As in other studies, the probability of same source pairs given examiners reported an inconclusive result is much lower than the expect 75\% based on the setup of the study.
...
\svp{The problems with ... ``The study was blind in that examiners did not know the true source of the cartridge cases."}
...
Results reported in the 2018 paper are shown at the top of \autoref{tab:hamby-summary}.
...
\multicolumn{5}{c}{\bf Experiment Count Data}\\
\multicolumn{1}{r}{} & Identification & Inconclusive & \multicolumn{1}{r}{Elimination} & Source Total\\\hline
Same Source & 10447 & 8 & 0.0000 & 10455 \\
Diff Source & 0.0000 & ? & ? & 47047.5\footnotemark \\\hline
...
Diff source & 0.0000 & ? & 1\footnotemark & 0.8282\\\hline
...
Diff source & 0.0000 & ? & ? & 0.8282\\\hline
\multicolumn{1}{Y}{Conclusion Probability} & 0.1817 & ? & \multicolumn{1}{r}{?} & \multicolumn{1}{Y}{}\\\hline\hline
\end{tabularx}
