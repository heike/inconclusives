...
In the context of \autoref{tab:fpr-prob} conditional probabilities can be interpreted as follows:
...
To reduce confusion with the naming or rates and predictive values we have labeled nodes in the trees of \autoref{fig:FPR-specific-tree} and \autoref{fig:PPV-specific-tree} corresponding to the rates defined in the text.
...
The decision-specific probabilities provide indications that the distribution of inconclusives is very different from its hypothesized  distribution shown in \autoref{fig:shape-overview}. %purposefully understated.
...
In \autoref{sec:dec-spec-res}, we refer to the expected distribution of inconclusives. The corresponding values are shown as grey targets in \autoref{fig:cis}.
...
In \autoref{fig:asymmetric-errors}, we see that in every experiment with sufficient data, the point estimate of the probability that an identification is from a different-source pair is higher than the probability that an elimination is from a same-source pair.
...
The study results are shown in \autoref{tab:lyons-summary}, along with computed conclusion-specific and source-specific probabilities and error rates.
...
As discussed in \autoref{sec:design-consequences}, closed-set studies like \citet{Hamby:2018hu} do not allow us to easily calculate the number of different-source comparisons performed. However, we can estimate the number of comparisons that might need to be performed using logic similar to that employed in \autoref{sec:bunchmurphysim} (but without the variability introduced by the unique experimental design in \citet{bunch2003comprehensive}).
