...
Mathematically, we would write the probability of an event $E$ conditional on another event $A$ as $P(E | A)$. Mathematically, conditional probability is a function of the probability of the both events: $P(E \text{ and } A) = P(E|A) P(A)$, so $$P(E|A) = \frac{P(E \text{ and } A)}{P(A)}.$$ That is, the probability of both $E$ and $A$ occurring is equal to the probability of $E$ given that $A$ has occurred, multiplied by the probability that $A$ has occurred. If there are only two possible outcomes, $A$ and $B$, then, using basic logic (or the law of total probability), we can write the probability of an event $E$ as $P(E) = P(E|A) P(A) + P(E|B) P(B)$. That is, if we know the probabilities of $A$ and $B$, and the probability of $E$ conditioned on both $A$ and $B$, we can calculate the probability of $E$ directly. 

\svp{Bayes rule}

During the analysis of the outcomes of examiner evaluations in a well-designed study, one might reasonably be interested in several different quantities. During the study design, experimenters determine the probability of a same-source evaluation (and the complementary probability of a different-source evaluation); this is a function of the study design. In our example above, a same-source evaluation might correspond to event $A$ and a different-source evaluation to event $B$. 
...
\begin{table}
\caption{An example results table for a generic experiment where comparisons can be broken down as either from the same source or from different sources, and examiners classify each comparison as an identification, an inconclusive, or an exclusion, as in the AFTE rules of identification.}\label{tab:generic-results}
\begin{tabular}{c|ccc|c}
& \multicolumn{3}{c}{Examiner Conclusion} &\\\cline{2-5}
...
\svp{Add in a table with the rates and how they are calculated using quantities in \autoref{tab:general-results}}
