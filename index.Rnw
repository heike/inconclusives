...
\svp{Nevertheless, in order for firearms and toolmark evidence to be admissible in court, we must be able to explicitly characterize the accuracy of the identification process \citep{giannelliDaubertInterpretingFederal1993}.}
...
\subsection{Studies and Design Considerations}
In order to assess the accuracy of the identification process, we have to make use of studies where ground truth is known. 
Ground truth is the knowledge of whether two samples come from the same source or from different sources. 
Assessment of error rates requires designed experiments where ground truth is known: in casework, where ground truth is unknown, it is not possible to assess the accuracy of the identification process.
...
\svp{In this paper, we will refer to studies which make use of participants who were not involved in the experimental design as ``single blind" - that is, the participant knows they are participating in a study, but does not know the answers to the study.
...
We will use the term ``double blind" to refer to situations where examiners do not know that the evidence being evaluated is part of a designed experiment.
}
...
\svp{In the remainder of this paper, we discuss the experimentally determined error rates reported in  studies in firearms and toolmark examination. 
...
\svp{It is unfortunate that t}he methods used to establish error rates experimentally and the interpretations of those error rates in the literature are highly variable. 
...
Under this framework, an examiner's assessment starts with inconclusive, \svp{and is} refined to identification or elimination given sufficient evidence in either direction.
...
\svp{I've changed my opinion on what should go in here a little bit - I think we want to set up the next section without making things too complicated. The (apocryphal) boiling a frog approach, if you will.}

\hh{There are essentially three ways that inconclusives can be treated in calculating error rates:}
(1) inconclusives can be excluded from result calculations, (2) included as correct results  \cite{Duez:2017kha}, or (3) included as incorrect results  \cite{chumbleyValidationToolMark2010}; each of these decisions has an impact on the \hh{actual value of the error rate as well as the} interpretation of the resulting error rates. \svp{I'm a bit mixed on whether this still needs to be here - it's a preview of the next section, but the point almost becomes repetitive... which may be good, if we're worried about comprehension, but may be boring if we're not.}
...
The accuracy of a classification method is  often expressed in the form of success (or accuracy) rates and error rates, that is, with numerical quantities indicating how often the process produces correct or incorrect results.
...
In firearms and toolmark examination, however, this is \svp{complicated by a mismatch between the classification scheme and the state of reality.} There are two possibilities to describe the physical state of the evidence, in that the evidence is either from the same source or from different sources, but there are three outcome states: the examiner can make an identification, an elimination, or an inconclusive determination.
\svp{Most (all?) of the typical methods of assessing classification decisions are designed for situations where there are the same number of possible decisions as there are physical classifications; 
}
