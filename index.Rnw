...
All three ``good" studies used slightly different designs, but maintained certain essentials: each kit consisted of multiple comparisons composed of one or more samples from exactly one known source and one unknown source. \svp{XXX can we phrase this to be singular, or cite another feature of the study (I would rather have it singular, personally) XXX}
...
In what is almost certainly a coincidence, all three studies are also limited to cartridge case comparisons. \svp{We could not identify any studies which assess the error rates of bullet or toolmark examination in a manner that would produce reliable and generalizable measurements.}
...
Of these, the study by  \citeauthor{bunch2003comprehensive} is perhaps the least flawed, \svp{but it has been superceded by studies that have better experimental design, such as \citet{Baldwin:2014bb} and \citet{keisler}.}
...
That is, the underlying structure of these studies only provides information about how accurate examiners are when providing evidence against the defendant, \svp{producing a systematic bias when these rates are presented in court}.
...
Finally, there are some \svp{design} constraints that are common to most studies. 
Of the studies assessed in this paper, only  \citet{bunch2003comprehensive} uses a variable \svp{proportion} of same and different source comparisons; without varying this percentage, it is possible that examiners could compare notes and gain additional information about the study results. 
