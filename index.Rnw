...
studies <- studies %>% mutate(
    Study = factor(Study, 
                    levels=c("Bunch & Murphy (2003)", "Lyons (2009)", "Hamby (2019)", "Baldwin (2014)", "Keisler (2018)", "Duez (2018)", "VCMER (2020)", "Mattijssen (2020)", "FAID2009 Bullets", "FAID2009 Cartridges"))
  )
...
gg %>%
  ...
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin =y - 0.3, ymax = y + 0.6), fill = "grey90", alpha = .25, inherit.aes = F, data = rects) + 
  ...
  geom_errorbarh(aes(xmin=err_lower, xmax=err_upper, colour = Type, 
                     y=as.numeric(Study)+0.3*(as.numeric(Type)-1.5)),
