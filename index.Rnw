...
\usepackage{tabu}\usepackage{tabularx}
...
Reality & Identification & Inconclusive & Exclusion & Comparison Totals \\\hline
...
\subsection{Conditional Error Rates}\label{sec:probabilities}
...
\item [P(same source \vbar examiner makes an exclusion)] The probability that two items come from the same source given that the examiner makes an exclusion. We will refer to this using the shorthand $P(SS|Exclusion)$. This is the second error that an examiner could make during an evaluation, and is termed a \emph{false negative} or a \emph{miss}. The probability or rate of such errors is termed the \emph{false negative rate} or \emph{miss rate}. 
\item [P(different source \vbar examiner makes an exclusion)] The probability that the two items come from different sources, given that the examiner makes an exclusion. We will refer to this using the shorthand  $P(DS|Exclusion)$. This is the second of two correct decisions which could be made during an evaluation and is known as a \emph{true negative} and the rate at which it occurs is known as the \emph{true negative rate} or the \emph{specificity}. 
\end{description}

Generic formulas for these probabilities can each be calculated from the quantities in \autoref{tab:generic-results}. A similar table is shown in \autoref{tab:generic-probabilities}, but instead of raw quantities describing evaluations, we have instead provided the conditional probabilities described at the beginning of this subsection (\autoref{sec:probabilities}). 

\begin{table}\centering
\caption{Probabilities of interest, calculated using the quantities introduced in \autoref{tab:generic-results}. In each cell in the main body of the table, probabilities shown are of $P(X | Y)$ where $X$ is the known type of comparison (e.g. reality), either same-source or different-source, and $Y$ is the examiner's decision (one of Identification, Inconclusive, or Exclusion. In the last row in the table, the marginal probabilities $P(Y)$ are shown, where $N$ is the total number of comparisons evaluated in the study. The joint probabilities (e.g. $P(X \text{ and } Y)$) described in \autoref{tab:generic-results} can be recovered from this table by multiplying any interior cell by the corresponding cell in the last row of the table. That is, $P(X \text{ and } Y) = P(X|Y) P(Y)$, which follows from \autoref{eqn:cond-prob}. }\label{tab:generic-probabilities}
{\tabulinesep=1.2mm
\begin{tabu}{|c|ccc|}\hline
$P(X | Y)$ & \multicolumn{3}{c|}{$Y = $Examiner Conclusion} \\\hline
$X = $Reality & Identification & Inconclusive & Exclusion \\\hline
\multirow{2}{*}{Same source} & $P(SS|Identification)$ & $P(SS|Inconclusive)$ & $P(SS | Exclusion)$ \\
& $=\displaystyle\frac{a}{a + d}$ & $=\displaystyle\frac{b}{b + e}$ & $=\displaystyle\frac{c}{c + f}$ \\\hline
\multirow{2}{*}{Different source} & $P(DS|Identification)$ & $P(DS|Inconclusive)$ & $P(DS | Exclusion)$\\ 
 & $=\displaystyle\frac{d}{a + d}$ & $=\displaystyle\frac{e}{b + e}$ & $=\displaystyle\frac{f}{c + f}$ \\\hline\hline
\multirow{2}{*}{$P(Y)$} & $P(Identification)$ & $P(Inconclusive)$& $P(Exclusion)$\\
...
\autoref{tab:error-rate-illustrated} shows several different possible error rate calculations, using the general structure of \autoref{tab:generic-results}.
...
\begin{table}\protect\caption{Different ways to calculate the overall error rate. An arrangement of two rows of three boxes then resembles the interior cells of \autoref{tab:generic-results}, where \pbox{10em}{\mbox{\whitesq{a}\whitesq{b}\whitesq{c}}\\\mbox{\whitesq{d}\whitesq{e}\whitesq{f}}} are the six conditional quantities under discussion. Let \pbox{1.65em}{\whitesq{}} indicate a correct decision, indicating that the box is to be included in the denominator of the calculation, \pbox{1.65em}{\graysq{}} indicate that the box is to be conditionally excluded from the calculation via \autoref{eqn:cond-prob}, and \pbox{1.65em}{\redsq{}} indicate an incorrect decision, indicating that the box is to be included in the numerator and denominator of the calculation. 
}
...
1 & \pbox[l]{8em}{Inconclusives\\as Correct} & \pbox{5em}{\whitesq{a}\whitesq{b}\redsq{c}\\\redsq{d}\whitesq{e}\whitesq{f}} & $P(DS\text{ and }Ident.) + P(SS\text{ and }Excl.) = \displaystyle\frac{c+d}{N}$\\\\
2 & \pbox[l]{8em}{Condition on\\Not Inconcl} & \pbox{5em}{\whitesq{a}\graysq{b}\redsq{c}\\\redsq{d}\graysq{e}\whitesq{f}} & $\displaystyle\frac{P(DS\text{ and }Ident.) + P(SS \text{ and } Excl.)}{P(Ident.) + P(Excl.)} = \displaystyle\frac{c + d}{a + d + c + f}$\\\\
...
In \autoref{tab:overall-err-rate}, option 0 corresponds to a definition of the overall error rate of a binary, symmetric decision process (2 actual states, 2 possible decisions), which for simplicity assumes that the two actual states are SS (same source) and DS (different source) and the two decisions are Identification and Exclusion. In this case, the overall error rate is the sum of the false positives and the false negatives, divided by the number of overall comparisons. However, if an inconclusive category is introduced which does not correspond to a real state, then we have to decide how to handle them. By AFTE's Theory of Identification, inconclusive classifications are not errors (option 1 in \autoref{tab:overall-err-rate}); however, this leads to an apparent contradiction, because \svp{XXX need to fix this when I'm not quite so tired}
Another reasonable solution would be to calculate the conditional probability of an error given that the classification was not inconclusive, as in option 2 of \autoref{tab:overall-err-rate}. Note that if we condition on the decision being either identification or exclusion, the probability calculations for scenarios 0 and 2 in \autoref{tab:overall-err-rate} are identical; the only difference between the two is the setup of the scenario: In \autoref{eqn:overall-error-conditional} we allow for inconclusives as a decision category but restrict them from the calculation of the error rate entirely. \svp{XXX need to explain scenario 3 and conclude it's the best option to cover the errors in the examination process as a whole.}

% What is unfortunately common in error rate studies, however, is a calculation of a different sort: Instead of calculating the conditional probability of an error given that the classification was not inconclusive, the inconclusives are included in the denominator of the calculation and excluded from the numerator. 
% \begin{equation}\label{eqn:overall-error-wrong}
% \text{(misleading)} P(Error) = P(Exclusion \text{ and } SS) + P(Identification \text{ and} DS) = \frac{c + d}{a + b + c + d + e + f}
% \end{equation}
% In \autoref{eqn:overall-error-wrong}, the conditional probabilities are reversed - instead of considering the probability of reality given the examiner decisions, the examiner's decision is conditioned on the known status of the comparison. This reversal is misleading in the treatment of inconclusives. To demonstrate this inconsistency, consider \autoref{tab:overall-error-misleading}, which has shaded cells corresponding to predictions which are not counted in the error rate. 
% 
% \begin{table}\centering
% \caption{A common method for calculating the error rate of forensic identifications when examiners can decide a comparison is an identification, inconclusive, or an exclusion. Rates are calculated 
% }\label{tab:overall-error-misleading}
% {\tabulinesep=1.2mm
% \begin{tabu}{|c|ccc||c|}\hline
% & \multicolumn{3}{c||}{Examiner Conclusion} &\\\cline{2-4}
% Reality & Identification & Inconclusive & Exclusion & Comparison Totals \\\hline
% Same source & $a$ & $b$ &\cellcolor{gray} $c$ & $S = a + b + c$ \\
% Different source & \cellcolor{gray}$d$ & $e$ & $f$ & $D = d + e + f$ \\\hline
% \end{tabu}}
% \end{table}


\svp{Introduce positive predictive value, e.g. \citet{med-pos-pred-value}}

In the next section, we will examine the results from a number of studies of error rates in a selection of commonly-cited firearms examination error rate studies. 

% What rates are reported in studies of examiner accuracy vary considerably: some studies report only the $P(SS|identification)$ accuracy and $P(DS|identification)$ accuracy, without considering any inconclusives or missed identifications; this may be a practical decision because the design of the study does not allow for the calculation of these rates, but in other cases, the information is just not reported at all. In other studies, inconclusive results are counted as correct identifications \citep{}

\subsection{Quantities of interest and practical significance}
...
$P(X | Y)$ & \multicolumn{3}{c|}{$Y = $Examiner Conclusion} \\\hline
$X = $Reality & Identification & Inconclusive & Exclusion \\\hline
Same source & $P(SS|Identification) = 1 $& $P(SS|Inconclusive) = P(SS)$ & $P(SS | Exclusion) = 0$ \\
Different source & $P(DS|Identification) = 0$ & $P(DS|Inconclusive) = P(DS)$ & $P(DS | Exclusion)= 1$ \\\hline
\end{tabu}
}
\end{table}
...
& \# Pairs & Elimination & \multirow{2}{1in}{Inconclusives or no response} & Identification\\
\\ \hline
...
\[
\begin{array}{lrrr}
P(X \mid Y) & \multicolumn{3}{c}{Y \text{ Expert conclusion}} \\
& \text{Identification} & \text{Inconclusive} & \text{Elimination} \\ \hline
X = \text{same source} &  \frac{1075}{1075+ 22} = 0.9799 & \frac{11}{737+11} = 0.0147 & \frac{4}{1421+4} =  0.0028 \\
X = \text{different source} & \\
\end{array}
\]
...
\begin{tabular}{lrrrr}\hline
& \# Pairs & Elimination & Inconclusives & Identification\\
\\ \hline
...
Combined, the probability for a  same source pair is 2/8 and the probability for  a different source  pair is 0.75.

\begin{tabular}{lrrr}\hline
Totals & Identifications & Inconclusives & Eliminations\\ \hline
same source & 335 & 1 & 0 \\
different source & 2 & 25 & 85 \\ \hline
\end{tabular}

\begin{tabular}{lrrr}\hline
Cond Prob & Identifications & Inconclusives & Eliminations\\ \hline
same source & 0.9941 & 0.0385 & 0.0000 \\
different source & 0.0059 & 0.9615 & 1.0000 \\ \hline
\end{tabular}

The probability of an inconclusive result is 26 out of 368, or 7.07\%.

\begin{tabular}{lrrr}\hline
Totals (Examiners) & Identifications & Inconclusives & Eliminations\\ \hline
same source & 276 & 0 & 0 \\
different source & 0 & 12 & 80 \\ \hline
\end{tabular}

\begin{tabular}{lrrr}\hline
Totals (Trainees) & Identifications & Inconclusives & Eliminations\\ \hline
same source & 59 & 13 & 0 \\
different source & 1 & 1 & 5 \\ \hline
\end{tabular}


``13\% of examiners are not permitted to eliminate on individual characteristics (therefore, their conclusions of inconclusive are perfectly acceptable)."

\hh{This is a very questionable conclusion.}
...
The study was originally reported on by Brundage in 1998 \cite{brundage} and updates on the study with increasing number of responses have been published several times since then \citet{Hamby:2018hu}. 
The design has also been copied in \citet{fadul} for cartridge cases.

Results reported in the 2018 paper: 
...
Technically, participants are asked to compare each questioned bullet  to each set of two knowns for a  
total of  \# questioned x \# different known source evaluations, resulting in  a total of 150 evaluations for each participant in the Brundage-Hamby study (15 same source  and 135 different source evaluations). 

However, this evaluations are not independent of each other: once a bullet is successfully identified to a set of knowns, none of the other sets of knowns can possibly be a match. Additionally, Brundage and Hamby studies do only ask participants to report identifications. We will therefore not consider any implicit eliminations of different source evaluations.  
For any inconclusive results, we know that firearms examiners could not identify the correct barrel, but at the same time we do not know how many different source evaluations would have resulted in inconclusives. 

\begin{eqnarray*}
\arraycolsep=1.4pt\def\arraystretch{2.2}
\begin{array}{rcccr}
P (\text{same source} \mid \text{identification made}) &=&  \frac{10455}{10455} &=& 1 \\
P (\text{different source} \mid \text{identification made}) &=&  \frac{0}{10455} &=& 0 \\[10pt]
P (\text{same source} \mid \text{inconclusive}) &=& \frac{8}{8 + \text{\#non-reported inconclusives for different source}} = ? \\
P (\text{different source} \mid \text{inconclusive}) &=& ? \\[10pt]
P (\text{same source} \mid \text{elimination made}) &=& ? \\
P (\text{different source} \mid \text{elimination made}) &=&  ? \\
\end{array}
\end{eqnarray*}
