...
studies <- studies %>% mutate(
    Study = factor(Study, 
                    levels=c("Bunch & Murphy (2003)", "Lyons (2009)", "Hamby (2019)", "Baldwin (2014)", "Keisler (2018)", "Duez (2018)", "VCMER (2020)", "Mattijssen (2020)", "FAID09 (2013) Bull.", "FAID09 (2013) Cartr."))
  )
...
gg %>%
  ...
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin =y - 0.5, ymax = y + 0.5),
            fill = "grey90", alpha = .5, inherit.aes = F, data = rects) +
...
errors2 <- studies %>% 
  ...
  ungroup() %>%
  mutate(
    afte = (Study %in% c("Mattijssen (2020)", "FAID2009 Bullets", "FAID2009 Cartridges")) %>% factor(levels = c(FALSE, TRUE), labels = c("AFTE scale", "Non-AFTE Scale"))
  )
...
errors2 %>%
  ...
  geom_hline(yintercept = "FAID2009 Bullets", colour = "black",
             size=16, alpha = 0.1) +
