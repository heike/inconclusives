...
studies <- studies %>% mutate(
    Study = factor(Study, 
                    levels=c("Baldwin (2014)", "Keisler (2018)", "Duez (2018)", "VCMER (2020)", "Mattijssen (2020)", "Bunch & Murphy (2003)", "Hamby (2019)", "Lyons (2009)", "FAID-2009 Bullets", "FAID-2009 Cartridges"))
  )
...
errors2 <- studies %>% 
  ...
  ungroup() %>%
  mutate(Study = str_replace(Study, "Matti", "*Matti"))
...
errors2 %>%
  ...
  geom_hline(yintercept = "Duez (2018)", colour = "black",
             size=16, alpha = 0.1) +
  geom_hline(yintercept = "VCMER (2020)", colour = "black",
             size=16, alpha = 0.1) +
...
in many studies, there are considerably more same-source comparisons than \svp{different-source comparisons}. Consequently, false elimination rates can be estimated more precisely than false identification rates. 
