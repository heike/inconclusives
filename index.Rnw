...
\autoref{tab:overall-err-rate} shows several different possible error rate calculations, using the general structure of \autoref{tab:generic-results}. Each of these calculations has a slightly different meaning and interpretation; as a result, it is important to consider which error rate best suits the purpose of any particular study.
...
Under AFTE's Theory of Identification, however, inconclusive classifications are not errors; an examiner could report inconclusives for the rest of their career and never make an error. In order to successfully accommodate this, we would likely calculate $P(Elimination | SS) = c/S$ as the false negative rate, and $P(Identification|DS) = d/D$ as the false positive rate (Note that the conditioning in these probabilities is flipped; instead of considering each column in the table as a separate group, these probabilities are conditioned on rows). Combining these two rates to get an overall error rate produces the equation for Option 1 in \autoref{tab:overall-err-rate}. What this means in real terms, however, is that inconclusives are effectively counted as identifications if they are actually from the same source, but counted as eliminations if they are actually from a different source. When the goal is to assess the error rate of the examiner under the prevailing guidelines of the AFTE Theory of Identification, this is a reasonable option. However, when viewed within the framework of the legal system, this error rate calculation is suboptimal. An examiner who marks every comparison as inconclusive does not provide useful information, but effectively has a zero error rate. From the wider perspective of the \emph{system}, it is more important to look at the overall error rate of the entire process. This requires that we consider the errors from the perspective of the real-world state: evidence is  either from the same source, or originates from different sources; an inability to distinguish between those options is an error in the \emph{process} even it if is not an error made on the part of the examiner. 


Reflecting this, option 3 of \autoref{tab:overall-err-rate} includes the inconclusives as errors. Practically, option 3 reflects the error of the examination process (rather than the examiner as an individual). To illustrate why this is the case, consider that an examiner may, upon inspection of a fired bullet, find that there are insufficient individualizing characteristics to make an identification. This does not reflective negatively on the examiner's skill; it is merely a consequence of the contact made between the bullet and the barrel during the firing process. Nonetheless, even though a pair of evidence originated from the same physical source, it is not possible to evaluate the evidence and make an identification. The inconclusives are representative of the errors involved in the process of evidence recording, rather than the evaluation. 
