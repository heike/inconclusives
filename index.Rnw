...
\svp{We have detailed several options for the treatment of inconclusive results when assessing error rates using traditional metrics such as sensitivity and specificity or when calculating an overall error rate}. 
There are, however, additional assessments of the reliability of a classification method which leverage \emph{conditional probabilities}: probabilities which are updated to account for known (or assumed) information.
...
\svp{Some of these} conditional probabilities have the advantage of not requiring inconclusives to be handled as errors or correct decisions. 
In addition, \svp{examining the} conditional probabilities offers some \svp{additional insights into the process of firearms examination and the types of errors which are likely to be made.}
