...
Many studies (but not all) pre-screened kit components to ensure that sufficient markings were present to allow for identification - this differs from casework and artifically inflates the rate of successful identifications and eliminations. 
...
That is, under ideal conditions, the expected conditional probabilities in an experiment would match those shown in \autoref{tab:ideal-probabilities}. 
}
...
\svp{As an example, in the Baldwin study  \cite{Baldwin:2014bb}}, \hh{we would expect the probabilities for same and different source given an inconclusive result to be: }
\begin{eqnarray*}
...
<<asymmetric-errors, echo = F, fig.width = 8, fig.height = 4, fig.cap = "" , eval = F>>=
...
elimplus <- studies %>%
  dplyr::filter(Decision != "Identification", Type == "Observed") %>%
  group_by(Study, Ground.truth) %>%
  summarize(Decision = "Elimination + Inconclusive", Number = sum(Number))
...
\svp{
Examining the experimentally determined error rates for firearms and toolmark identification, however, led us to a disturbing realization. 
...
\svp{
In machine learning and statistics, the equal error rate is frequently used to set cutoff thresholds, because at the equal error rate, a false positive and a false negative are equally likely.
In common law, Blackstone's ratio (``It is better that ten guilty persons escape than that one innocent suffer.") would suggest that the equal error rate is insufficient. 
Under this principle, found across legal traditions, ideally examiners would be less likely to make a false identification than a false elimination. 
}
\svp{XXX The EER is stated in source-specific terms. I'm not sure it holds for decision specific stuff. Thoughts? Is it ok to make the comparison, even if it's not precise?}

\svp{
%Looking at the predictive value of an inconclusive result led us to the following unexpected (and ugly) realization: 
Essentially, examiners are willing to gamble on an indentification much more than they are willing to gamble on an elimination. 
In the courtroom, this results in a clear bias toward the prosecution (or against the defense).
}

\subsection{The Effect of Inconclusive Policies}
