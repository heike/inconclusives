...
Inconclusives can be excluded from result calculations, included as correct results\cite{Duez:2017kha}, or included as incorrect results\cite{chumbleyValidationToolMark2010}; each of these decisions has an impact on the interpretation of the resulting error rates.
...
examining previous error rate studies to assess the technical accuracy and practical utility of the \svp{error rates which reported in error rate studies of firearms and toolmark examination}. 
...
source_spec_cis %>% 
  ...
  theme_bw() +
  geom_col(aes(x = Study, y = I(1)), width = 0.7, colour = "grey", alpha = 0.1, fill = "grey", linetype = "solid") + 
