...
> Ok, so we got caught :-p How do we fix it though?

> Thank you for pointing out this inconsistency - we tried to define error rates conceptually first before getting into the more contentious areas of what constitutes an error. 
> We have rephrased the first paragraph to:
...
> Ok, so it sounds like we need to elaborate a bit on this. It's not inconsistent, it's just that we're making the point of bias in a roundabout way.

> HH: I think what contributes to the problem that we implicitly talk about Fig 7 and the error rates from Baldwin on the previous page, and only explicitly mention Figure 8, which does not show the order of magnitude difference (directly).
> Proposed fix: 

- add reference to figure 7, add equation number to error rates, refer to both. (done) 
- Start a new paragraph before mentioning figure 8 (done)
- Insert a paragraph to explain the need for a sufficient sample size to see a difference between 0.02 and 0.2 (done) 
...
> I think the reviewer missed that we're basically saying Keisler wasn't a big enough study (iirc)
...
> Isn't this part of the NAS report? I'm going to check...

- I've changed this to say different-source comparisons - the point remains valid, and that I can support with evidence (VCMER, Baldwin, and Bunch/murphy) vs. everything else. 
...
- Thanks for catching that, we must have checked the numbers 10x or more, but errors still sneak in!
